...
$staffFilter = ($isExecutive || $isAdmin)
    ? ((int)($_GET['staff_id'] ?? 0) ?: null)
    : $uid;
$selectedBranch = ($isExecutive || $isAdmin)
    ? ((int)($_GET['branch'] ?? 0) ?: 0)
    : $branchId;
...
/* ──────────────────────────────────────────────────────────────
   WHO VISITED (executive/admin only)
────────────────────────────────────────────────────────────── */
$whoVisited = [];
if ($isExecutive || $isAdmin) {
    $whoVisited = $db->query("
        ...
        <table class="table-mis w-100">
            <thead>
                <tr>
                    ...
                    <th style="min-width:120px;">Efficiency</th>
                </tr>
            </thead>
            <tbody>
            <?php if (empty($monthlyData)): ?>
...
<?php /* ══════════════════ WHO VISITED TAB ═══════════════════ */ elseif ($view === 'who' && ($isExecutive || $isAdmin)): ?>
