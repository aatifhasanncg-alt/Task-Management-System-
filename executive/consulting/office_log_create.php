...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($rows as $idx => $row) {
        ...
        $rowErr = [];
        if (!$clientId)     $rowErr[] = 'Please select a client.';
        if (!$logDate)      $rowErr[] = 'Log date is required.';
        if (!$description)  $rowErr[] = 'Description is required.';
        if (!$timeIn)       $rowErr[] = 'Start Time is required.';
        if (!$timeOut)      $rowErr[] = 'End Time is required.';

        if ($timeIn && $timeOut) {
            $diff = strtotime($timeOut) - strtotime($timeIn);
            if ($diff <= 0) $rowErr[] = 'End Time must be after Start Time.';
        }
