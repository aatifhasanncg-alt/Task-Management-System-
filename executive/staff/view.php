...
$taskStmt = $db->prepare("
    SELECT
        ...
        SUM(CASE WHEN ts.status_name = 'WIP'     THEN 1 ELSE 0 END) AS wip_tasks,
        SUM(CASE WHEN ts.status_name = 'Pending' THEN 1 ELSE 0 END) AS pending_tasks,
        SUM(CASE WHEN t.due_date < CURDATE()
            AND ts.status_name != 'Done'         THEN 1 ELSE 0 END) AS overdue_tasks
    FROM tasks t
        ...
        <div class="card-mis mb-4">
            ...
            <div class="card-mis-body">
                <div class="row g-3">
                    ...
                    <!-- Status -->
                    <div class="col-md-4">
                        <div style="font-size:.7rem;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em;">Status</div>
                        <div style="margin-top:.2rem;">
                            <?php if ($staffUser['is_active']): ?>
                                <span style="background:#ecfdf5;color:#10b981;padding:.2rem .65rem;border-radius:99px;font-size:.78rem;font-weight:600;">
                                    <i class="fas fa-circle me-1" style="font-size:.5rem;"></i>Active
                                </span>
                            <?php else: ?>
                                <span style="background:#fef2f2;color:#ef4444;padding:.2rem .65rem;border-radius:99px;font-size:.78rem;font-weight:600;">
                    ...
                    <!-- 2FA Status -->
                    <div class="col-md-4">
                        <div style="font-size:.7rem;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em;">2FA Status</div>
                        <div style="margin-top:.2rem;">
                            ...
                            <?php else: ?>
                                <span style="background:#fff7ed;color:#f59e0b;padding:.2rem .65rem;border-radius:99px;font-size:.78rem;font-weight:600;">
        ...
        <div class="card-mis mb-4">
            ...
            <div class="card-mis-body">
                <div class="row g-3 text-center">
                    <?php foreach ([
                        ['Total',   $taskStats['total_tasks'],   '#3b82f6', '#eff6ff',  'fa-list-check'],
                        ['WIP',     $taskStats['wip_tasks'],     '#f59e0b', '#fffbeb',  'fa-spinner'],
                        ['Pending', $taskStats['pending_tasks'], '#ef4444', '#fef2f2',  'fa-clock'],
                        ['Done',    $taskStats['done_tasks'],    '#10b981', '#ecfdf5',  'fa-check-circle'],
                        ['Overdue', $taskStats['overdue_tasks'], '#dc2626', '#fef2f2',  'fa-exclamation-circle'],
                    ...
                    <div class="col">
                        <div style="background:<?= $bg ?>;border-radius:10px;padding:.75rem .5rem;">
                            ...
                            <div style="font-size:1.4rem;font-weight:700;color:<?= $color ?>;line-height:1.2;margin-top:.2rem;">
                                <?= $val ?? 0 ?>
                            </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card-mis mb-4">
            <div class="card-mis-header">
                ...
                <?php if ($staffUser['ga_enabled']): ?>
                    <span style="background:#ecfdf5;color:#10b981;padding:.2rem .55rem;border-radius:99px;font-size:.72rem;font-weight:600;">Active</span>
                <?php else: ?>
                    <span style="background:#fff7ed;color:#f59e0b;padding:.2rem .55rem;border-radius:99px;font-size:.72rem;font-weight:600;">Pending</span>
                <?php endif; ?>
            </div>
            <div class="card-mis-body text-center">
                ...
                <?php if (!empty($staffUser['ga_secret'])): ?>
                    <!-- QR Code -->
                    <div style="background:#fff;padding:12px;border-radius:10px;border:2px solid #e5e7eb;display:inline-block;margin-bottom:.75rem;">
                        <img src="<?= htmlspecialchars($qr) ?>"
                    ...
                    <!-- Secret Key -->
                    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:.65rem;text-align:left;">
                        <div style="font-size:.7rem;color:#9ca3af;margin-bottom:.3rem;text-transform:uppercase;font-weight:600;">Secret Key</div>
                    ...
                    <?php if (!$staffUser['ga_enabled']): ?>
                    <div style="margin-top:.75rem;background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:.6rem .75rem;font-size:.78rem;color:#92400e;text-align:left;">
        ...
        <div class="card-mis">
            ...
            <div class="card-mis-body">
                <a href="reset_password.php?id=<?= $staffId ?>"
