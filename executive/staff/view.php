...
$udaDeptStmt = $db->prepare("
    ...
    WHERE uda.user_id = ?
    UNION
    SELECT d.id, d.dept_name, d.color, 1 AS is_primary
    FROM departments d
    WHERE d.id = ?
...
// Deduplicate
$seen = [];
$allStaffDepts = array_values(array_filter($allStaffDepts, function ($d) use (&$seen) {
...
if (!empty($allStaffDepts)) {
    $deptIds = array_column($allStaffDepts, 'id');
    $inList = implode(',', array_fill(0, count($deptIds), '?'));
    ...
    $dtsStmt = $db->prepare("
        ...
        GROUP BY t.department_id, ts.status_name, ts.color, ts.bg_color
        ORDER BY ts.id
    ");
    $dtsStmt->execute(array_merge([$staffId], $deptIds));

    foreach ($dtsStmt->fetchAll() as $row) {
        $did = $row['department_id'];
        $deptTaskStats[$did][$row['status_name']] = [
            'cnt' => (int) $row['cnt'],
    ...
    <div class="main-content">
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="row g-4">
                ...
                letter-spacing:.05em;margin-bottom:.5rem;">Departments</div>
                                        ...
                                        <table style="width:100%;border-collapse:collapse;font-size:.82rem;">
                                            <thead>
                                                <tr style="background:#f9fafb;">
                                                    <th style="padding:.35rem .6rem;font-size:.68rem;color:#9ca3af;font-weight:700;
                           text-transform:uppercase;border-bottom:1px solid #e5e7eb;">#</th>
                                                    <th style="padding:.35rem .6rem;font-size:.68rem;color:#9ca3af;font-weight:700;
                           text-transform:uppercase;border-bottom:1px solid #e5e7eb;">Department</th>
                                                    <th style="padding:.35rem .6rem;font-size:.68rem;color:#9ca3af;font-weight:700;
                           text-transform:uppercase;border-bottom:1px solid #e5e7eb;">Type</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                        ...
                        <div class="card-mis mb-4">
                            ...
                            <div class="card-mis-body">
                                <?php foreach ($allStaffDepts as $dept):
                                    $did = $dept['id'];
                                    $dColor = $dept['color'] ?: '#9ca3af';
                                    $statuses = $deptTaskStats[$did] ?? [];
                    ...
                    margin-bottom:.75rem;border-left:4px solid <?= htmlspecialchars($dColor) ?>;">
                                ...
                                background:<?= htmlspecialchars($dColor) ?>;transition:.3s;"></div>
                                            </div>
                                            <div style="font-size:.68rem;color:#9ca3af;text-align:right;margin-top:.2rem;">
                                                <?= $donePct ?>% done · <?= $doneCnt ?>/<?= $deptTotal ?>
                                            </div>
