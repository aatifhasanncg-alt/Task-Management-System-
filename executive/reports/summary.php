...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_summary'])) {
    ...
    if ($existingRow) {
        ...
        $db->prepare("
            ...
            WHERE id=?
        ")->execute([
            $totalFiles, $completed, $hbc, $pending,
            $support, $cancelled, $isChecked, $user['id'],
        ...
        $db->prepare("
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?)
        ")->execute([
            $bankRefId, $branchId, $fy,
            $totalFiles, $completed, $hbc, $pending, $support, $cancelled,
            $isChecked, $user['id']
        ]);
    }

    setFlash('success', 'Bank summary updated.');
    header('Location: summary.php?branch_id=' . $branchId . '&fiscal_year=' . urlencode($fy)); exit;
}

// ── Fetch summary rows ──────────────────────────────────────────────────────
$where  = ['1=1'];
$params = [];
if ($filterBranch) { $where[] = 'bs.branch_id = ?';  $params[] = $filterBranch; }
if ($filterFY)     { $where[] = 'bs.fiscal_year = ?'; $params[] = $filterFY; }
...
foreach ($summary as &$row) {
    // percentage of THIS bank's files out of ALL banks' total files
        ...
        <table class="table-mis w-100" id="bankSummaryTable">
            <thead>
                <tr style="background:#0a0f1e;">
                    ...
                    <th style="color:#c9a84c;text-align:center;" title="Bank's total files ÷ Grand total × 100">% of Total Files</th>
            ...
            <tbody>
                ...
                <tr style="background:#1e2a45;">
                    ...
                    <td style="color:#c9a84c;font-weight:700;text-align:center;"><?= $totalPct ?>%</td>
                ...
                <?php
                    /*
                     * Per-row percentage formula:
                     *   pct = (this_bank_total_files / grand_total_all_banks) * 100
                     *
                     * Example: X=2, Y=3, Z=5 → grand=10
                     *   X → 2/10*100 = 20%
                     *   Y → 3/10*100 = 30%
                     *   Z → 5/10*100 = 50%
                     *   Sum of all rows = 100% ✓
                     */
                    $pct = $row['pct_of_total_files'];  // already computed above
                ?>
                <tr>
                    <td style="color:#9ca3af;font-size:.78rem;"><?= $i + 1 ?></td>
                    <td style="font-size:.87rem;font-weight:500;"><?= htmlspecialchars($row['bank_name'] . (isset($row['bank_address']) && $row['bank_address'] ? ' - ' . $row['bank_address'] : '')) ?></td>
                    ...
                    <td style="text-align:center;">
                        <span style="background:#ecfdf5;color:#10b981;padding:.2rem .5rem;border-radius:4px;font-size:.78rem;font-weight:600;">
                            <?= $row['completed'] ?>
                        </span>
                    </td>
                    <td style="text-align:center;">
                        <span style="background:#fffbeb;color:#f59e0b;padding:.2rem .5rem;border-radius:4px;font-size:.78rem;font-weight:600;">
                            <?= $row['hbc'] ?>
                        </span>
                    </td>
                    <td style="text-align:center;">
                        <span style="background:#fef2f2;color:#ef4444;padding:.2rem .5rem;border-radius:4px;font-size:.78rem;font-weight:600;">
                            <?= $row['pending'] ?>
                        </span>
                    </td>
                    <td style="text-align:center;font-size:.78rem;"><?= $row['support'] ?></td>
                    <td style="text-align:center;">
                        <span style="background:#fef2f2;color:#ef4444;padding:.2rem .5rem;border-radius:4px;font-size:.78rem;">
                            <?= $row['cancelled'] ?>
                        </span>
                    </td>
                    <td style="text-align:center;">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" style="background:#0a0f1e;">
                <h5 class="modal-title text-white"><i class="fas fa-landmark me-2"></i>Update Bank Summary</h5>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_bank_select" class="form-select form-select-sm"
                                ...
                                <?php foreach ($allBanks as $b): ?>
                                    <option value="<?= $b['id'] ?>"><?= htmlspecialchars($b['bank_name'] . ($b['address'] ? ' - ' . $b['address'] : '')) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label-mis">Branch</label>
                            <select name="branch_id" id="edit_branch" class="form-select form-select-sm" required>
                                <?php foreach ($allBranches as $b): ?>
                        ...
                        <div class="col-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox"
                                       name="is_checked" id="edit_is_checked">
                                <label class="form-check-label" for="edit_is_checked">Mark as Checked (TRUE)</label>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    <div class="table-responsive">
        <table class="table-mis w-100">
            ...
            <tbody>
                ...
                <tr><td colspan="4" class="empty-state"><i class="fas fa-university"></i> No banks added yet</td></tr>
                <?php endif; ?>
                <?php foreach ($allBanks as $i => $bank): ?>
                <tr>
                    <td style="color:#9ca3af;font-size:.78rem;"><?= $i + 1 ?></td>
                    <td style="font-size:.87rem;font-weight:500;" colspan="2"><?= htmlspecialchars($bank['bank_name'] . ($bank['address'] ? ' - ' . $bank['address'] : '')) ?></td>
                    <td style="text-align:center;">
                        <button class="btn btn-sm btn-outline-secondary"
...
<div class="modal fade" id="editBankRefModal" tabindex="-1">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                <button class="btn btn-gold btn-sm" onclick="document.getElementById('editBankRefForm').submit();">
