<?php
// admin/reports/staff_wise.php
require_once '../../config/db.php';
require_once '../../config/config.php';
require_once '../../config/session.php';
requireExecutiveOrBM();
...
// Correct the WHERE conditions for branch and department filters
// ── User scope (determines which staff rows appear) ───────────────────────────
$selfId = $user['id'];
...
if (isExecutive()) {
    ...
    // BM: all staff in their branch, skip CORE dept admins
...
// Date params first (used in the JOIN), then user scope params
$staffStmt->execute(array_merge(
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="row g-3 mb-4">
                <?php foreach ([
                    ...
                    <div class="col-6 col-md-3">
                        <div
                            style="background:#fff;border-radius:12px;border:1px solid #f3f4f6;padding:1.1rem 1.2rem;display:flex;align-items:center;gap:.9rem;">
                            <div
                                style="width:42px;height:42px;border-radius:10px;background:<?= $bg ?>;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            ...
            <div class="card-mis mb-4" style="border-left:4px solid #10b981;">
                <div class="card-mis-body" style="padding:.9rem 1.2rem;">
                        ...
                        <div style="font-size:.82rem;font-weight:600;color:#374151;">
                            ...
                            <?php if ($topPerformer): ?>
                                <span style="font-size:.72rem;color:#9ca3af;font-weight:400;margin-left:.75rem;">
                                    Top: <strong
                                        style="color:#10b981;"><?= htmlspecialchars(explode(' ', $topPerformer['full_name'])[0]) ?></strong>
                    ...
                    <div style="background:#f3f4f6;border-radius:99px;height:7px;margin-top:.6rem;">
                        <div
                            style="width:<?= $overallPct ?>%;background:linear-gradient(90deg,#10b981,#34d399);height:100%;border-radius:99px;transition:width .6s ease;">
                        </div>
                    </div>
                    <div style="font-size:.7rem;color:#9ca3af;margin-top:.3rem;"><?= $totalDone ?> of <?= $totalTasks ?>
                        tasks completed across <?= $totalStaff ?> staff</div>
                </div>
            </div>
            ...
            <div class="filter-bar mb-4 w-100">
                <form method="GET" class="row g-2 align-items-end w-100">
                    ...
                    <div class="col-md-2"><label class="form-label-mis">Department</label>
                    ...
                    <?php if (!$isBranchManager): ?>
                        <div class="col-md-2"><label class="form-label-mis">Branch</label>
                    ...
                    <table class="table-mis w-100">
                        ...
                        <tbody>
                            <?php if (empty($staffReport)): ?>
                                <tr>
                                    <td colspan="<?= 5 + count($allStatuses) ?>" class="empty-state"><i
                                            class="fas fa-users"></i> No staff data found</td>
                                </tr>
                            <?php endif; ?>
                                ...
                                <tr style="<?= $s['id'] == $selfId ? 'background:#eff6ff;' : '' ?>">
                                    ...
                                    <td>
                                        <div
                                            ...
                                            <div style="display:flex;align-items:center;gap:.6rem;">
                                                <div
                                                    style="width:34px;height:34px;border-radius:50%;background:<?= htmlspecialchars($s['dept_color'] ?? '#c9a84c') ?>22;color:<?= htmlspecialchars($s['dept_color'] ?? '#c9a84c') ?>;display:flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:700;flex-shrink:0;">
                                                    <?= $initials ?>
                                                </div>
                                                <div>
                                                    <div style="font-size:.87rem;font-weight:500;color:#1f2937;display:flex;align-items:center;gap:6px;">
                                                        <?= htmlspecialchars($s['full_name']) ?>

                                                        <?php if ($s['id'] == $selfId): ?>
                                                            <span style="font-size:.65rem;background:#3b82f6;color:#fff;padding:2px 6px;border-radius:10px;">
                                                                YOU
                                                            </span>
                                                        <?php endif; ?>
                                                    </div>
                                            ...
                                            <a href="<?= APP_URL ?>/admin/staff/view.php?id=<?= $s['id'] ?>"
                                                ...
                                                style="padding:.2rem .45rem;flex-shrink:0;"><i class="fas fa-eye"
                                    ...
                                    <?php foreach ($allStatuses as $st):
                                        ...
                                        <td class="text-center">
                                            <?php if ($count > 0): ?>
                                                <span
                                                    style="background:<?= htmlspecialchars($st['bg_color']) ?>;color:<?= htmlspecialchars($st['color']) ?>;padding:.15rem .45rem;border-radius:99px;font-size:.72rem;font-weight:600;display:inline-block;"><?= $count ?></span>
                                        ...
                                        <?php if ($s['total'] > 0): ?>
                                            <div style="display:flex;align-items:center;gap:.5rem;">
                                                <div
                                                    ...
                                                    <div
                                                        style="width:<?= $pct ?>%;background:<?= $barColor ?>;height:100%;border-radius:99px;transition:width .4s;">
                                                    </div>
                                                </div>
                                                <span
                                                    style="font-size:.72rem;font-weight:700;color:<?= $barColor ?>;white-space:nowrap;min-width:38px;text-align:right;"><?= $pct ?>%</span>
                                            </div>
                                            <div style="font-size:.65rem;color:#9ca3af;margin-top:.15rem;">
                                                <?= $s['done'] ?>/<?= $s['total'] ?> done
                                                <?php if ($s['overdue'] > 0): ?>&nbsp;<span
                                                        style="color:#ef4444;font-weight:600;">⚠ <?= $s['overdue'] ?>
                                                        overdue</span><?php endif; ?>
                                            </div>
                                        <?php else: ?><span style="font-size:.75rem;color:#9ca3af;">No
