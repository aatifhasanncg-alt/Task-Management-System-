...
if (isExecutive()) {
    ...
    if ($filterDept)   { $userWhere[] = "(u.department_id = ? OR EXISTS (
    ...
    // Regular admin: locked to their branch, limited to their accessible depts
    $userWhere[]  = 'u.branch_id = ?';
    $userParams[] = $adminBranchId;
    if (!empty($udaDeptIds)) {
...
if ($filterDept) $joinParams[] = $filterDept;
...
foreach ($staffReport as $s) {
    ...
    if ($pct > $topPct) { $topPerformer = $s; $topPct = $pct; }
}
...
.role-badge {
    ...
    font-weight:600;border-radius:20px;text-transform:capitalize;
}
.role-admin     { background:#dbeafe;color:#1d4ed8;border:1px solid #93c5fd; }
.role-executive { background:#e0e7ff;color:#3730a3; }
            ...
            <tbody>
                ...
                <tr style="<?= $s['id'] == $selfId ? 'background:#eff6ff;' : '' ?>">
                    ...
                    <td class="text-center">
                        <span style="font-size:.9rem;font-weight:700;color:#1f2937;"><?= $s['total'] ?></span>
                    </td>
                    ...
                    <td style="min-width:160px;">
                        ...
                        <div style="font-size:.65rem;color:#9ca3af;margin-top:.15rem;">
                            <?= $s['done'] ?>/<?= $s['total'] ?> done
                            <?php if ($s['overdue'] > 0): ?>
                            &nbsp;<span style="color:#ef4444;font-weight:600;">⚠ <?= $s['overdue'] ?> overdue</span>
                            <?php endif; ?>
                        </div>
                        <?php else: ?>
