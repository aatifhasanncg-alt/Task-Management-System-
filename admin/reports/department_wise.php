...
$udaDeptStmt = $db->prepare("SELECT department_id FROM user_department_assignments WHERE user_id = ?");
$udaDeptStmt->execute([$user['id']]);
$udaDeptIds = array_column($udaDeptStmt->fetchAll(PDO::FETCH_ASSOC), 'department_id');

$statusRows = $db->query("SELECT id, status_name, color, bg_color, icon FROM task_status ORDER BY id ASC")->fetchAll();
...
// Build branch filter for BM
$branchFilter = '';
$branchParams = [];
if ($isBranchManager && $bmBranchId) {
    $branchFilter = 'AND t.branch_id = ?';
    $branchParams[] = $bmBranchId;
}
$deptFilter = '';
if (!$isBranchManager && !isExecutive() && !empty($udaDeptIds)) {
    $placeholders = implode(',', array_fill(0, count($udaDeptIds), '?'));
    $deptFilter   = "AND t.department_id IN ($placeholders)";
    foreach ($udaDeptIds as $did) $branchParams[] = $did;
}
$deptStmt = $db->prepare("
    ...
    ORDER BY total DESC
");
$deptStmt->execute(array_merge([$fromDate . ' 00:00:00', $toDate . ' 23:59:59'], $branchParams));
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="page-hero">
                <div class="d-flex justify-content-between flex-wrap gap-2 align-items-center">
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-center">
                        <a href="index.php" class="btn btn-outline-light btn-sm"><i
            ...
            <div class="filter-bar mb-4 w-100">
                <form method="GET" class="row g-3 align-items-end w-100">
                    <div class="col-md-2"><label class="form-label-mis">From</label><input type="date" name="from"
                            class="form-control form-control-sm" value="<?= $fromDate ?>"></div>
                    <div class="col-md-2"><label class="form-label-mis">To</label><input type="date" name="to"
                            class="form-control form-control-sm" value="<?= $toDate ?>"></div>
                    ...
                    <div class="dept-card-body">
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
                            <div class="d-flex align-items-center gap-3">
                                <div
                                    style="width:48px;height:48px;border-radius:12px;background:<?= htmlspecialchars($d['color']) ?>22;color:<?= htmlspecialchars($d['color']) ?>;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:.75rem;color:#9ca3af;margin-top:.15rem;">
                                        ...
                                        <?php if ($d['overdue'] > 0): ?>
                                            &nbsp;·&nbsp;<span style="color:#ef4444;font-weight:600;"><i class="fas fa-clock"
                                                    style="font-size:.65rem;"></i> <?= $d['overdue'] ?> overdue</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                <div style="text-align:right;">
                                    <div
                                        style="font-size:1.6rem;font-weight:800;color:<?= htmlspecialchars($d['color']) ?>;line-height:1;">
                                        <?= $pct ?>%
                                    </div>
                                ...
                                <div style="position:relative;">
                                    <svg width="68" height="68" viewBox="0 0 68 68">
                                        <circle cx="34" cy="34" r="<?= $r ?>" fill="none" stroke="#f3f4f6"
                                            stroke-width="6" />
                                    ...
                                    <div
                                        style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:.62rem;font-weight:700;color:<?= htmlspecialchars($d['color']) ?>;">
                                        <?= $doneVal ?>/<?= $d['total'] ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php if ($d['total'] > 0): ?>
                            <div style="background:#f3f4f6;border-radius:99px;height:6px;overflow:hidden;margin-bottom:1rem;">
                                <div
                                    style="width:<?= $pct ?>%;background:<?= htmlspecialchars($d['color']) ?>;height:100%;border-radius:99px;transition:width .4s;">
                                </div>
                            </div>
                        <?php endif; ?>
                        ...
                        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:.6rem;">
                            <?php foreach ($statusRows as $sr):
                                ...
                                <a href="<?= $tileLink ?>" class="status-tile"
                                    style="background:<?= htmlspecialchars($bg) ?>;border-color:<?= htmlspecialchars($color) ?>33;">
                                    <div
                                    ...
                                    <div
                                        style="font-size:1.35rem;font-weight:800;color:<?= htmlspecialchars($color) ?>;line-height:1.1;">
                                        <?= $val ?>
                                    </div>
                                    <div
                                        style="font-size:.65rem;color:#6b7280;margin-top:.2rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            ...
                            <a href="<?= APP_URL ?>/admin/tasks/index.php?<?= http_build_query([
                                  ...
                                  'date_from' => $fromDate,
                                  'date_to' => $toDate,
                              ]) ?>" class="status-tile" style="background:#fef2f2;border-color:#fecaca;">
                                <div
                                    style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.3rem;">
                                ...
                                <div style="font-size:.65rem;color:#6b7280;margin-top:.2rem;font-weight:600;">Overdue</div>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <?php if (empty($deptReport)): ?>
                <div class="card-mis text-center py-5" style="color:#9ca3af;">
                    <i class="fas fa-layer-group fa-2x mb-2 d-block"></i>No department data for this period.
                </div>
            <?php endif; ?>
            ...
            <div class="card-mis mb-4">
                ...
                <div class="card-mis-body">
                    <div style="height:320px;position:relative;"><canvas id="deptChart"></canvas></div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const datasets = meta.map(m => ({
            label: m.label, data: data.map(d => parseInt(d[m.key] ?? 0)),
            backgroundColor: m.color, borderRadius: 3, borderSkipped: false,
        }));
        new Chart(document.getElementById('deptChart'), {
            type: 'bar',
            data: { labels: data.map(d => d.dept_name), datasets: datasets },
            options: {
                    ...
                    tooltip: {
                        callbacks: {
                            footer: function (items) {
                                const idx = items[0]?.dataIndex; const total = parseInt(data[idx]?.total ?? 0);
                ...
                scales: {
                    ...
                    y: { stacked: true, beginAtZero: true, grid: { color: '#f3f4f6' }, ticks: { font: { size: 11 }, stepSize: 1 }, title: { display: true, text: 'Tasks', font: { size: 11 }, color: '#9ca3af' } }
                }
            }
        });
    });
