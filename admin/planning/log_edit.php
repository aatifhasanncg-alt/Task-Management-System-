...
$logId = (int)($_GET['id'] ?? 0);
if (!$logId) { setFlash('error', 'Invalid log.'); header('Location: log_list.php'); exit; }
            ...
            if (!empty($user['managed_by'])) {
                try {
                    ...
                    ")->execute([
                        ...
                        APP_URL . '/admin/planning/log_list.php?month=' . $month,
                    ]);
                } catch (Exception $ne) {}
            }
...
.hrs-display    { background:var(--cn3);border-radius:8px;padding:10px 14px;text-align:center; }
.vst-option     { display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:8px;
                  border:2px solid transparent;cursor:pointer;transition:all .15s;font-size:.82rem;font-weight:600; }
.vst-option:hover { border-color:currentColor;opacity:.85; }
.vst-option.active { border-color:currentColor; }
.info-row       { display:flex;align-items:center;justify-content:space-between;padding:6px 0;
                  border-bottom:1px solid var(--cn4);font-size:.78rem; }
.info-row:last-child { border-bottom:none; }
.info-label     { color:var(--muted);min-width:120px; }
                            ...
                            <div style="padding:16px 18px;">
                                ...
                                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:14px;">
                                    <div>
                                        ...
                                        <div id="dayLabel" style="font-size:.72rem;color:var(--muted);margin-top:4px;font-weight:600;">
                                            <?= $log['day_of_week'] ?>
                                        </div>
                                    </div>
                                    <div>
                    ...
                    <div>
                        ...
                        <div class="cn-panel" style="margin-bottom:14px;">
                            ...
                            <div style="padding:14px 16px;display:flex;flex-direction:column;gap:8px;">
                                ...
                                <a href="log_list.php?month=<?= $month ?>"
