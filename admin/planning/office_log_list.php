...
$statusFilter   = in_array($_GET['status'] ?? '', ['', 'wip', 'completed']) ? ($_GET['status'] ?? '') : '';
...
// ── WHERE ──────────────────────────────────────────────────────────────────
$where  = "DATE_FORMAT(owl.log_date,'%Y-%m') = ? AND owl.department_id = ? AND owl.user_id IN ({$inList})";
$params = [$month, $deptId];
...
.log-cards-head {
    ...
    padding: .9rem 1.2rem; border-bottom: 1px solid var(--border); gap: .75rem; flex-wrap: wrap;
}
.log-cards-head h5 { font-size: .92rem; font-weight: 700; color: var(--ink); margin: 0; }
.filtered-tag { font-size: .7rem; font-weight: 700; color: var(--gold); background: var(--gold-lt); padding: .18rem .55rem; border-radius: 6px; }

/* ── Day group ───────────────────────────────────────────────────────── */
.day-group { padding: .85rem 1.2rem 0; }
.day-header {
...
.day-today-tag {
    font-size: .62rem; font-weight: 700; background: #fef3c7; color: #d97706;
    padding: .1rem .4rem; border-radius: 5px;
}
.day-line { flex: 1; height: 1px; background: var(--border); }
.day-meta { font-size: .7rem; color: var(--muted); white-space: nowrap; }
...
.office-card:hover { box-shadow: 0 3px 14px rgba(15,23,42,.08); transform: translateY(-1px); }
.office-card-accent {
    position: absolute; left: 0; top: 0; bottom: 0; width: 3px;
    border-radius: 10px 0 0 10px;
}
.office-card-body { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
.office-card-left { flex: 1; min-width: 0; padding-left: .5rem; }
.office-card-right { display: flex; align-items: center; gap: 1rem; flex-shrink: 0; }

.client-name { font-size: .9rem; font-weight: 700; color: var(--ink); }
.client-code {
    font-size: .67rem; background: #f1f5f9; color: var(--muted);
    border-radius: 5px; padding: .1rem .4rem; font-weight: 600;
}
.staff-tag { font-size: .72rem; color: var(--muted); }
.staff-tag i { color: var(--gold); }

.off-badge {
    font-size: .67rem; font-weight: 700; padding: .18rem .52rem; border-radius: 6px;
    display: inline-flex; align-items: center; gap: .28rem;
}
.off-badge i { font-size: .58rem; }

.log-desc {
    font-size: .79rem; color: var(--muted); margin-top: .4rem;
    overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 580px;
}
.log-notes { font-size: .72rem; color: #d97706; margin-top: .22rem; }
.log-notes i { margin-right: .2rem; }

.time-block { text-align: center; }
.time-hours { font-size: 1.1rem; font-weight: 800; color: var(--gold); line-height: 1; }
.time-range { font-size: .65rem; color: var(--muted); white-space: nowrap; margin-top: .18rem; }
...
.log-summary-bar .sum-val { color: var(--ink); font-weight: 800; }
...
@media (max-width: 768px) {
    ...
    .log-desc { max-width: 240px; }
    .office-card-right { width: 100%; justify-content: space-between; border-top: 1px solid var(--border); padding-top: .65rem; margin-top: .5rem; }
}
</style>
        ...
        <div style="padding:1.5rem 1rem 2rem;">
            ...
            <div class="log-cards-wrap">
                <div class="log-cards-head">
                    <h5><i class="fas fa-building me-2" style="color:var(--gold);"></i>Office Logs — <?= $monthLabel ?></h5>
                    <div style="display:flex;align-items:center;gap:.65rem;">
                        <?php if ($hasFilters): ?><span class="filtered-tag"><i class="fas fa-filter me-1"></i>Filtered</span><?php endif; ?>
                        <span style="font-size:.74rem;color:var(--muted);"><?= $totalLogs ?> records · <?= number_format($totalHours,1) ?>h total</span>
                    </div>
                </div>
                ...
                <?php else: ?>
                                    ...
                                    <div class="office-card-body">
                                            ...
                                            <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;margin-bottom:.3rem;">
                                                ...
                                                <span class="staff-tag">
                                                    ...
                                                    <?php if (!empty($l['employee_id'])): ?>
                                                        <span style="color:var(--border);margin:0 .2rem;">·</span><?= htmlspecialchars($l['employee_id']) ?>
                                                    <?php endif; ?>
                                                </span>
                                            </div>
                                        ...
                                        <div class="office-card-right">
                                            ...
                                            <div style="display:flex;gap:5px;">
                                                <a href="office_log_view.php?id=<?= $l['id'] ?>" class="act-btn"
                                                   title="View" style="background:#ecfeff;border-color:#a5f3fc;color:#0e7490;">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <?php if ((int)$l['user_id'] === $uid): ?>
                                                    <a href="office_log_edit.php?id=<?= $l['id'] ?>" class="act-btn"
                                                       title="Edit" style="background:#fefce8;border-color:#fde68a;color:#92400e;">
                    ...
                    <div class="log-summary-bar">
                        ...
                        <span>Hours: <span class="sum-val" style="color:#b45309;"><?= number_format($totalHours,1) ?>h</span></span>
                        <span>WIP: <span class="sum-val" style="color:#2563eb;"><?= (int)($kpi['wip_count'] ?? 0) ?></span></span>
                        <span>Completed: <span class="sum-val" style="color:#059669;"><?= (int)($kpi['completed_count'] ?? 0) ?></span></span>
