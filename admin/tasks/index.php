...
if ($filterDateFrom) { $where[] = 't.created_at >= ?';   $params[] = $filterDateFrom . ' 00:00:00'; }
if ($filterDateTo)   { $where[] = 't.created_at <= ?';   $params[] = $filterDateTo   . ' 23:59:59'; }
if ($filterCompany)  { $where[] = 't.company_id = ?';    $params[] = $filterCompany; }
if ($filterDept)     { $where[] = 't.department_id = ?'; $params[] = $filterDept; }
...
foreach ($allStatuses as $st) {
    ...
    if ($filterStatus) {
        ...
        if ($idx !== false) { unset($tabParams[$idx]); $tabParams = array_values($tabParams); }
    }
    $cntSt = $db->prepare("
        SELECT COUNT(DISTINCT t.id)
        FROM tasks t
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="d-flex gap-2 flex-wrap mb-3">
                ...
                ?>
                    <a href="?<?= http_build_query(array_merge($_GET, ['status'=>$k,'page'=>1])) ?>"
                        ...
                        <?= htmlspecialchars($k) ?>
                        <span style="background:<?= $active ? 'rgba(255,255,255,.25)' : $col ?>33;color:<?= $active ? '#fff' : $col ?>;padding:.05rem .4rem;border-radius:99px;font-weight:700;">
                            <?= $tabCounts[$k] ?? 0 ?>
                        </span>
                    </a>
                <?php endforeach; ?>
            ...
            <div class="filter-bar mb-4 w-100">
                <form method="GET" class="row g-2 align-items-end w-100">
                    <?php if ($showAll && !$isBranchManager): ?><input type="hidden" name="show_all" value="1"><?php endif; ?>
                    <?php if ($filterStatus): ?><input type="hidden" name="status" value="<?= htmlspecialchars($filterStatus) ?>"><?php endif; ?>
                    <?php if ($filterCompany): ?><input type="hidden" name="company_id" value="<?= $filterCompany ?>"><?php endif; ?>
                    <div class="col-md-3">
            ...
            <!-- Edit hint -->
            <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:.7rem 1rem;margin-bottom:1.25rem;display:flex;align-items:flex-start;gap:.75rem;">
                ...
                <div style="font-size:.8rem;color:#92400e;">
                    ...
                    <span style="display:inline-flex;align-items:center;gap:.3rem;margin-top:.3rem;">
                        <span style="background:#0a0f1e;color:#c9a84c;padding:.15rem .5rem;border-radius:5px;font-size:.72rem;font-weight:600;"><i class="fas fa-eye me-1"></i>View Page</span>
                        — fill dept-specific fields and update work status
                    </span><br>
                    <span style="display:inline-flex;align-items:center;gap:.3rem;margin-top:.3rem;">
                        <span style="background:#f59e0b;color:#fff;padding:.15rem .5rem;border-radius:5px;font-size:.72rem;font-weight:600;"><i class="fas fa-pen me-1"></i>Edit Page</span>
            ...
            <div class="card-mis">
                <div class="card-mis-header">
                    ...
                    <div class="d-flex gap-2">
                        <a href="<?= APP_URL ?>/exports/export_excel.php?module=tasks&<?= http_build_query(array_merge($_GET, ['branch_id'=>$adminBranchId,'dept_id'=>$adminDeptId])) ?>"
                            class="btn btn-sm" style="background:#16a34a;color:white;border-radius:8px;padding:.35rem .8rem;">
                            <i class="fas fa-file-excel me-1"></i>Excel
                        </a>
                        <a href="<?= APP_URL ?>/exports/export_pdf.php?module=tasks&<?= http_build_query($_GET) ?>"
                            class="btn btn-sm" style="background:#dc2626;color:white;border-radius:8px;padding:.35rem .8rem;">
                ...
                <div class="table-responsive">
                    <table class="table-mis w-100">
                        ...
                        <tbody>
                            <?php if (empty($tasks)): ?>
                                <tr><td colspan="9" class="empty-state"><i class="fas fa-list-check"></i> No tasks found</td></tr>
                            <?php endif; ?>
                            ...
                            <tr <?= $overdue ? 'style="background:#fef2f2;"' : '' ?>>
                                <td style="min-width:100px;">
                                    <span class="task-number" style="font-size:.75rem;"><?= htmlspecialchars($t['task_number']) ?></span>
                                    <?php if ($overdue): ?><div style="font-size:.62rem;color:#ef4444;font-weight:700;margin-top:.1rem;">⚠ OVERDUE</div><?php endif; ?>
                                    <?php if ($transferredIn): ?>
                                        <div style="font-size:.62rem;font-weight:600;color:#8b5cf6;margin-top:.15rem;"><i class="fas fa-arrow-down me-1"></i>IN</div>
                                        <div style="font-size:.6rem;color:#9ca3af;">From: <span style="color:<?= htmlspecialchars($t['transfer_from_dept_color'] ?? '#8b5cf6') ?>;font-weight:600;"><?= htmlspecialchars($t['transfer_from_dept_name'] ?? '—') ?></span></div>
                                    <?php elseif ($transferredOut): ?>
                                        <div style="font-size:.62rem;font-weight:600;color:#f59e0b;margin-top:.15rem;"><i class="fas fa-arrow-up me-1"></i>OUT</div>
                                        <div style="font-size:.6rem;color:#9ca3af;">To: <span style="color:<?= htmlspecialchars($t['transfer_to_dept_color'] ?? '#f59e0b') ?>;font-weight:600;"><?= htmlspecialchars($t['transfer_to_dept_name'] ?? '—') ?></span></div>
                                    <?php elseif ($hasTransfer): ?>
                                        <div style="font-size:.62rem;color:#6b7280;margin-top:.15rem;"><i class="fas fa-exchange-alt me-1"></i>Transferred</div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div style="font-weight:500;font-size:.87rem;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($t['title']) ?></div>
                                    <?php if ($t['company_name']): ?><div style="font-size:.73rem;color:#9ca3af;"><?= htmlspecialchars($t['company_name']) ?></div><?php endif; ?>
                                </td>
                                <td>
                                    <span style="font-size:.73rem;background:<?= htmlspecialchars($t['dept_color'] ?? '#ccc') ?>22;color:<?= htmlspecialchars($t['dept_color'] ?? '#666') ?>;padding:.2rem .5rem;border-radius:99px;font-weight:500;">
                                        <?= htmlspecialchars($t['dept_name'] ?? '—') ?>
                                    </span>
                                    <?php if (!empty($t['branch_name'])): ?><div style="font-size:.68rem;color:#9ca3af;margin-top:.1rem;"><?= htmlspecialchars($t['branch_name']) ?></div><?php endif; ?>
                                </td>
                                <td style="font-size:.85rem;"><?= htmlspecialchars($t['assigned_to_name'] ?? '—') ?></td>
                                <td><span class="status-badge <?= $sClass ?>"><?= htmlspecialchars($t['status'] ?? '—') ?></span></td>
                                <td>
                                    <?php $pColors = ['urgent'=>'#ef4444','high'=>'#f59e0b','medium'=>'#3b82f6','low'=>'#9ca3af']; $pColor = $pColors[$t['priority']] ?? '#9ca3af'; ?>
                                    <span style="font-size:.78rem;font-weight:600;color:<?= $pColor ?>;"><?= ucfirst($t['priority']) ?></span>
                                </td>
                                <td style="font-size:.8rem;<?= $overdue ? 'color:#ef4444;font-weight:600;' : 'color:#6b7280;' ?>"><?= $t['due_date'] ? date('d M Y', strtotime($t['due_date'])) : '—' ?></td>
                                <td style="font-size:.78rem;color:#9ca3af;"><?= htmlspecialchars($t['fiscal_year'] ?? '—') ?></td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="view.php?id=<?= $t['id'] ?>" class="btn btn-sm btn-outline-secondary" title="View"><i class="fas fa-eye"></i></a>
                                        <a href="edit.php?id=<?= $t['id'] ?>" class="btn btn-sm btn-outline-warning" title="Edit"><i class="fas fa-pen"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
