...
if (!isCoreAdmin()) {
    setFlash('error', 'Access denied. Only Administrator can manage grades.');
    header('Location: ' . APP_URL . '/admin/dashboard/index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if ($min === '' || !is_numeric($min)) $errors[] = 'Min Revenue is required and must be a number.';
    if ($max !== '' && !is_numeric($max)) $errors[] = 'Max Revenue must be a number.';
    ...
    <div style="background:#fff;border-radius:16px;padding:1.75rem;
        ...
        <form method="POST" id="grade-form">
            ...
            <div class="row g-2 mb-3">
                <div class="col-6">
                    <label class="form-label-mis">Min Revenue <span style="color:#ef4444;">*</span></label>
                    <input type="number" step="0.01" name="min_profit" id="form-min"
                           class="form-control" placeholder="0.00">
                </div>
                <div class="col-6">
                    <label class="form-label-mis">Max Revenue <span style="color:#ef4444;">*</span></label>
