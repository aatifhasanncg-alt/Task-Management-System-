<?php
/**
 * manager/consulting/branch/log_list.php — Manager: All Staff Visit Logs + Office Logs
 */
...
// ── Manager is locked to their own branch — ignore any other branch_id from query string ──
$managerBranchId = (int) ($user['branch_id'] ?? 0);
...
$branchList = $db->query("
    ...
    FROM branches
    WHERE is_active = 1 AND id = {$managerBranchId}
...
$staffList = $db->query("
    ...
    FROM users u
    WHERE u.is_active = 1
      AND u.branch_id = {$managerBranchId}
      AND (
          u.id = {$uid}
          OR u.id IN (
...
$sql1 = "
    ...
    LEFT JOIN work_plan_entries rpe ON rpe.id = wl.rescheduled_to_entry_id
            ...
            <div class="cn-wrap">
                ...
                <form method="GET" class="filter-bar">
                    ...
                    <div style="display:flex;align-items:center;gap:6px;flex:1;min-width:180px;">
                        ...
                        <select name="branch_id" id="branchSelect" style="width:100%;" disabled>
                    ...
                    <div id="fieldStatusWrap" style="display:flex;align-items:center;gap:6px;">
                        ...
                        <select name="field_status" id="fieldStatusSelect" class="cn-input" style="width:145px;">
                            ...
                            <option value="rescheduled" <?= $fieldStatus === 'rescheduled' ? 'selected' : '' ?>>🔄
                                Rescheduled
                            </option>
                        </select>
                    </div>
                    ...
                    <div id="officeStatusWrap" style="display:flex;align-items:center;gap:6px;">
                        ...
                        <select name="office_status" id="officeStatusSelect" class="cn-input" style="width:140px;">
                            <option value="">All</option>
                            <option value="not_started" <?= $officeStatus === 'not_started' ? 'selected' : '' ?>>▶️ Not
                                Started
                            </option>
                        ...
                        <table class="cn-table w-100" id="logsTable">
                                    ...
                                    <?php foreach ($logs as $l):
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div style="font-size:.68rem;color:#9ca3af;"><?= $l['day_of_week'] ?? '' ?>
                                                </div>
                                            </td>
                                            <td>
                            ...
                            <?php if (!empty($logs)):
                                ...
                                <tfoot>
                                 ...
                                 font-size:.68rem;font-weight:700;padding:2px 7px;border-radius:99px;">
                                                                <i class="fas fa-spinner" style="font-size:.55rem;"></i>
                                                                <?= $totWip ?>
                                                                WIP
                                                            </span>
                                                        <?php endif; ?>
            ...
            <div class="log-modal-body">
                ...
                <div id="modalRescheduleWrap" style="display:none;margin-top:15px;">
                    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:12px;">
                        <div style="font-size:.8rem;font-weight:700;color:#b45309;margin-bottom:10px;">
                            <i class="fas fa-redo me-1"></i> Rescheduled Details
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function openModal(data) {
                ...
                /* Reschedule details */
                const rescheduleWrap = document.getElementById('modalRescheduleWrap');
                const rescheduleContent = document.getElementById('modalRescheduleContent');
                if (data.visit_status === 'rescheduled' && data.reschedule_date) {
                    rescheduleContent.innerHTML =
                        '<div class="log-detail-grid">' +
                        '<div class="log-detail-item"><label>New Visit Date</label><div class="val">' + escapeHtml(data.reschedule_date) + '</div></div>' +
                        '<div class="log-detail-item"><label>Planned Hours</label><div class="val">' + escapeHtml(data.reschedule_hours || '-') + 'h</div></div>' +
                        '<div class="log-detail-item"><label>Time In</label><div class="val">' +
                        (data.reschedule_time_in ? new Date('2000-01-01 ' + data.reschedule_time_in).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : '-') +
                        '</div></div>' +
                        '<div class="log-detail-item"><label>Time Out</label><div class="val">' +
                        (data.reschedule_time_out ? new Date('2000-01-01 ' + data.reschedule_time_out).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : '-') +
                        '</div></div>' +
                        '</div>' +
                        (data.reschedule_notes
                            ? '<div style="margin-top:10px;"><label style="font-size:.7rem;color:#9ca3af;">Notes</label><div class="log-desc-box">' + escapeHtml(data.reschedule_notes) + '</div></div>'
                            : '');
