<?php
/**
 * consulting/manager/branch/client_report.php — Manager: Client-wise Performance Report
 *
...
// ── Branch Manager (CORE dept) detection ───────────────────────
$__deptCodeStmt = $db->prepare("SELECT dept_code FROM departments WHERE id = ?");
$__deptCodeStmt->execute([$deptId]);
$isBranchManager = ($__deptCodeStmt->fetchColumn() === 'CORE');

$selectedBranch = $isBranchManager
    ? $branchId
    : ((isset($_GET['branch']) && $_GET['branch'] !== 'all') ? (int) $_GET['branch'] : 0);
...
$clientWhereExtra = '';
if ($filterClientId)
    $clientWhereExtra .= " AND wl.client_id = {$filterClientId}";
if ($filterStatus)
    $clientWhereExtra .= " AND wl.visit_status = " . $db->quote($filterStatus);
...
$unvisitedClients = $db->query("
    ...
    WHERE c.is_active = 1
      ...
      AND c.id NOT IN (
          SELECT DISTINCT wl.client_id FROM work_logs wl
          WHERE wl.month_year = '{$month}'
            AND wl.user_id IN ({$inList})
            {$branchWhere}
      )
...
<div class="app-wrapper">
    ...
    <div class="main-content">
        ...
        <div style="padding:1.5rem 0;">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div>
                        <div class="page-hero-badge">
                            <i class="fas fa-building"></i> Manager · Client Report
                            <?php if ($notifCount > 0): ?>
                        ...
                        <p>
                            ...
                            <?php if ($filterBranchId): ?>
                                <span
                                    style="background:#eff6ff;color:#3b82f6;border-radius:99px;padding:.1rem .55rem;font-size:.72rem;margin-left:.3rem;">
                                    <?php foreach ($branches as $b)
                                        if ($b['id'] == $filterBranchId)
                                            echo htmlspecialchars($b['branch_name']); ?>
                                </span>
                            <?php else: ?>
                                <span
                                    style="background:#f3f4f6;color:#6b7280;border-radius:99px;padding:.1rem .55rem;font-size:.72rem;margin-left:.3rem;">
                                    All Branches
                                </span>
                            <?php endif; ?>
                        </p>
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-center">
                        <input type="month" class="form-control form-control-sm" style="width:150px;"
                            value="<?= $month ?>"
                            onchange="location='?month='+this.value+'&branch=<?= $_GET['branch'] ?? 'all' ?>'">
                        <a href="plans.php" class="btn btn-outline-secondary btn-sm position-relative">
                ...
                <div class="card-mis-body" style="padding:.75rem 1rem;">
                    <div class="row g-2 align-items-end">
                        <div class="col-md-1">
                            ...
                            <input type="month" id="fMonth" class="form-control form-control-sm" value="<?= $month ?>"
                        ...
                        <?php else: ?>
                            <div class="col-md-2">
                                <label class="form-label-mis">Branch</label>
                                <div style="font-size:.82rem;font-weight:600;color:#c9a84c;padding:.35rem 0;">
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="date" id="fFrom" class="form-control" value="<?= $filterFrom ?>"
                                    onchange="applyFilters()">
                                <input type="date" id="fTo" class="form-control" value="<?= $filterTo ?>"
                                    onchange="applyFilters()">
                            </div>
                        </div>
                        <div class="col-md-1">
            ...
            <!-- ══ KPI CARDS — FIELD ══════════════════════════════════════ -->
            <div
                style="font-size:.7rem;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;">
            ...
            <div class="row g-3 mb-3">
                ...
                box-shadow:0 1px 3px rgba(0,0,0,.04);">
                    ...
                    display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            ...
                            <div>
                                <div style="font-size:1.2rem;font-weight:800;color:#1f2937;line-height:1.1;"><?= $val ?>
                                </div>
            ...
            <!-- ══ KPI CARDS — OFFICE ══════════════════════════════════════ -->
            <div
                style="font-size:.7rem;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;margin-top:.25rem;">
                ...
                <div class="col-md-4">
                    <div class="card-mis h-100">
                        <div class="card-mis-header">
                            ...
                            <span style="font-size:.78rem;color:#9ca3af;"><?= (int) ($officeKpi['total_logs'] ?? 0) ?>
                                total</span>
                        </div>
                        <div class="card-mis-body">
                            <?php
                            ...
                            foreach ($offStatusRows as [$lbl, $v, $c]):
                                ...
                                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;font-size:12px;">
                                    <span
                                        style="width:80px;font-size:11px;font-weight:500;color:#6b7280;"><?= $lbl ?></span>
                                    <div style="flex:1;height:7px;background:#f3f4f6;border-radius:99px;overflow:hidden;">
                                        <div style="width:<?= $w ?>%;background:<?= $c ?>;height:100%;border-radius:99px;">
                                        </div>
                                    </div>
                                    <span
                                        style="width:28px;text-align:right;font-weight:600;color:#1f2937;font-size:12px;"><?= $v ?></span>
                                </div>
                            <?php endforeach; ?>
                                ...
                                <div style="margin-top:1rem;border-top:1px solid #f3f4f6;padding-top:.75rem;">
                                    ...
                                    foreach (array_slice($monthlyOffice, 0, 5) as $mo):
                                        ...
                                        <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;font-size:11px;">
                                            <span
                                                style="width:70px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#6b7280;">
                                                <?= htmlspecialchars(explode(' ', $mo['full_name'])[0]) ?>
                                            </span>
                                            <div
                                                style="flex:1;height:5px;background:#f3f4f6;border-radius:99px;overflow:hidden;">
                                                <div
                                                    style="width:<?= $opct ?>%;background:#10b981;height:100%;border-radius:99px;">
                                                </div>
                                            </div>
                                            <span
                                                style="font-size:.67rem;color:#10b981;font-weight:600;min-width:30px;text-align:right;">
            ...
            if ($filterClientId) {
                foreach ($clientPerf as $cp) {
                    if ($cp['client_id'] == $filterClientId) {
                        $selClient = $cp;
                        break;
                    }
                }
                ...
                <div class="card-mis mb-4" style="border-left:4px solid #3b82f6;">
                    <div class="card-mis-header">
                        ...
                        <div style="display:flex;align-items:center;gap:.5rem;">
                            <span
                                style="font-size:.75rem;color:#9ca3af;"><?= htmlspecialchars($selClient['branch_name'] ?? '') ?></span>
                                    ...
                                    <tbody>
                                        <?php foreach ($drilldownLogs as $dl): ?>
                                            <tr>
                                                ...
                                                <td style="font-size:.74rem;color:#8b5cf6;">
                                                    <?= htmlspecialchars($dl['branch_name'] ?? '') ?>
                                                </td>
                                                <td>
                                    ...
                                    <tbody>
                                        <?php foreach ($unloggedPlans as $up): ?>
                                            <tr style="background:#fffbeb;">
                                                ...
                                                <td style="font-size:.74rem;color:#8b5cf6;">
                                                    <?= htmlspecialchars($up['branch_name'] ?? '') ?>
                                                </td>
                        ...
                        <table class="table-mis w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th style="min-width:120px;">Visit Match %</th>
                                    <th>First · Last</th>
                                    <th>Drill</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($clientPerf as $i => $cp):
                                    ...
                                    <tr <?= $isSelected ? 'style="background:#eff6ff;"' : '' ?>>
                                        ...
                                        <td>
                                            <span
                                                style="font-size:.73rem;background:#f5f3ff;color:#8b5cf6;padding:.1rem .4rem;border-radius:6px;">
                                        ...
                                        <td class="text-center">
                                            <span
                                                style="background:#f0fdf4;color:#15803d;padding:2px 8px;border-radius:20px;font-size:.74rem;font-weight:600;">
                                                <?= (int) $cp['visited'] ?>
                                            </span>
                                        </td>
                                        <td class="text-center">
                                            <?php if ((int) $cp['missed'] > 0): ?>
                                                <span
                                                    style="background:#fef2f2;color:#b91c1c;padding:2px 8px;border-radius:20px;font-size:.74rem;font-weight:600;">
                                                    <?= (int) $cp['missed'] ?>
                                                </span>
                                        ...
                                        <td class="text-center">
                                            <?php if ((int) $cp['rescheduled'] > 0): ?>
                                                <span
                                                    style="background:#fffbeb;color:#b45309;padding:2px 8px;border-radius:20px;font-size:.74rem;font-weight:600;">
                                                    <?= (int) $cp['rescheduled'] ?>
                                                </span>
                                            <?php else: ?><span style="color:#d1d5db;font-size:.77rem;">—</span><?php endif; ?>
                                        </td>
                                            ...
                                            <?php if ((float) $cp['planned_hours'] > 0): ?>
                                                <div style="display:flex;align-items:center;gap:.35rem;">
                                                    <div
                                                        ...
                                                        <div
                                                            style="width:<?= $cEff ?>%;background:<?= $cEffCol ?>;height:100%;border-radius:99px;">
                                                        </div>
                                                    </div>
                                                    <span
                                                        style="font-size:.7rem;font-weight:700;color:<?= $cEffCol ?>;min-width:32px;text-align:right;"><?= $cEff ?>%</span>
                                                </div>
                                                <?php if ($cEffRaw > 100): ?>
                                                    <div style="font-size:.6rem;color:#f59e0b;">⚠ <?= $cEffRaw ?>% raw</div>
                                                <?php endif; ?>
                                            <?php else: ?><span style="font-size:.72rem;color:#9ca3af;">No
                                                    plan</span><?php endif; ?>
                                        </td>
                                        <!-- Visit efficiency -->
                                        <td>
                                            <?php if ((int) $cp['planned_entries'] > 0): ?>
                                                <div style="display:flex;align-items:center;gap:.35rem;">
                                                    <div
                                                        style="flex:1;background:#f1f5f9;border-radius:99px;height:5px;overflow:hidden;">
                                                        <div
                                                            style="width:<?= $vcEff ?>%;background:<?= $vcEffCol ?>;height:100%;border-radius:99px;">
                                                        </div>
                                                    </div>
                                                    <span
                                                        style="font-size:.7rem;font-weight:700;color:<?= $vcEffCol ?>;min-width:32px;text-align:right;"><?= $vcEff ?>%</span>
                                                </div>
                    ...
                    <div class="card-mis-body">
                        <div style="display:flex;flex-wrap:wrap;gap:.5rem;">
                            <?php foreach ($unvisitedClients as $uv): ?>
                                <div
                                    style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:.4rem .8rem;font-size:.78rem;">
                                    <span
                                        style="font-weight:600;color:#92400e;"><?= htmlspecialchars($uv['company_name']) ?></span>
                                    <span
                                        style="color:#b45309;font-size:.68rem;margin-left:.3rem;"><?= htmlspecialchars($uv['company_code'] ?? '') ?></span>
                                    <?php if (!$filterBranchId): ?>
...
<script>
    ...
    new Chart(document.getElementById('trendChart'), {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: true, grid: { color: '#f3f4f6' }, ticks: { font: { size: 10 }, callback: v => v + 'h' },
                    title: { display: true, text: 'Hours', font: { size: 10 } }
                },
                y1: {
                    beginAtZero: true, position: 'right', grid: { display: false }, ticks: { font: { size: 10 } },
    ...
    function applyFilters() {
        ...
        const p = new URLSearchParams({
            ...
            branch: branchEl ? branchEl.value : '<?= $isBranchManager ? $branchId : "all" ?>',
        });
