<?php
/**
 * consulting/manager/branch/dashboard.php — Manager Dashboard
 */
...
$__deptCodeStmt->execute([$user['department_id']]);
$isBranchManager = ($__deptCodeStmt->fetchColumn() === 'CORE');

// Branch managers are locked to their own branch; everyone else keeps
// the existing branch-selector behavior.
$selectedBranch = $isBranchManager ? (string) $branchId : ($_GET['branch'] ?? 'all');
...
$staffPerf = $db->query("
    ...
    WHERE u.is_active=1
      AND (
          ...
          OR
          /* in CON dept (manager sees all CON staff, branch-manager scoped to own branch) */
          u.id IN (
              SELECT u2.id FROM users u2
              JOIN departments d ON d.id=u2.department_id AND d.dept_code='CON'
              WHERE u2.is_active=1
              " . ($isBranchManager ? "AND u2.branch_id = {$branchId}" : "") . "
              UNION
              ...
              JOIN users u3 ON u3.id = uda2.user_id
              " . ($isBranchManager ? "WHERE u3.branch_id = {$branchId}" : "") . "
          )
      )
...
<style>
    /* ═══════════════════════════════════════════
   MANAGER DASHBOARD — LAYOUT RESET
            ...
            <div class="page-hero mb-4">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-center">
                        ...
                        <?php else: ?>
                            <span style="font-size:.78rem;color:#c9a84c;font-weight:600;background:#fdf6e3;
