<?php
/**
 * app url/manager/consulting/plan_view.php — Manager: View & Approve/Reject a Work Plan
 *
 * Fixes:
 *  ✓ All three subqueries (logged_count, logged_hours, logged_status) now use
 *    DATE(wl.log_date) = wpe.plan_date for consistent matching
 *  ✓ logged_hours no longer uses an inconsistent date range comparison
 *  ✓ Uses getDepartmentStaff() approach for multi-dept staff scope
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve' && $plan['status'] === 'submitted') {
        ...
        notify(
            ...
            'task',
            APP_URL . '/manager/consulting/plan_view.php?id=' . $planId,
            false,
            []
        );

        // Badge counter on dashboard
        $db->prepare("
        ...
        logActivity('Manager approved plan #' . $planId, 'consulting', 'plan_id=' . $planId);
    ...
    if ($action === 'reject' && $remarks && $plan['status'] === 'submitted') {
        $db->prepare("
            UPDATE work_plans
            SET status='rejected', approved_by=?, approved_at=NULL, remarks=?
            WHERE id=?
        ...
        logActivity('Manager rejected plan #' . $planId, 'consulting', 'plan_id=' . $planId);
...
// FIX: All three correlated subqueries now use DATE(wl.log_date) = wpe.plan_date
// consistently so that any logged visit on that date is detected correctly.
$stmt = $db->prepare("
    SELECT wpe.*,
           c.company_name, c.company_code,
           u.full_name AS assigned_name,

           /* How many work_logs exist for this client+user on this plan date */
           (SELECT COUNT(*)
            FROM work_logs wl
           ...
           /* Total hours logged for this client+user on this plan date */
           ...
           /* Visit status of the first log for this client+user on this plan date */
           ...
           /* Actual time_in for this client+user on this plan date */
           (SELECT wl.time_in
            FROM work_logs wl
            WHERE wl.client_id  = wpe.client_id
              AND wl.user_id    = wpe.assigned_to
           ...
           /* Actual time_out for this client+user on this plan date */
           (SELECT wl.time_out
            FROM work_logs wl
            WHERE wl.client_id  = wpe.client_id
              AND wl.user_id    = wpe.assigned_to
...
$matchedCount  = count(array_filter($entries, fn($e) => (int)$e['logged_count'] > 0));
...
// Hour efficiency
$hourEff      = $totalPlannedH > 0 ? min(100, round(($totalActualH / $totalPlannedH) * 100)) : 0;
$hourEffColor = $hourEff >= 80 ? '#10b981' : ($hourEff >= 50 ? '#f59e0b' : '#ef4444');

// Visit status counts
$vstCounts = ['visited' => 0, 'missed' => 0, 'rescheduled' => 0];
foreach ($entries as $e) {
    if ($e['logged_status'] && isset($vstCounts[$e['logged_status']])) {
        $vstCounts[$e['logged_status']]++;
    }
}
...
<div class="page-hero mb-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <div class="page-hero-badge"><i class="fas fa-briefcase"></i> Manager · Plan Review</div>
        ...
        <div class="card-mis mb-3">
            ...
            <div class="card-mis-body">
                <?php foreach ([
                    ...
                    ['Visited',       $vstCounts['visited'],                      '#10b981'],
                    ['Missed',        $vstCounts['missed'],                       '#ef4444'],
                    ['Rescheduled',   $vstCounts['rescheduled'],                  '#f59e0b'],
                ] as [$lbl,$val,$col]): ?>
                ...
                <div style="margin-top:.6rem;">
                    <div style="display:flex;justify-content:space-between;font-size:.75rem;margin-bottom:.3rem;">
                        <span style="color:#9ca3af;">Hour Efficiency</span>
                        <strong style="color:<?= $hourEffColor ?>;"><?= $hourEff ?>%</strong>
                    </div>
                    <div style="background:#f3f4f6;border-radius:99px;height:7px;overflow:hidden;">
                        <div style="width:<?= $hourEff ?>%;background:<?= $hourEffColor ?>;height:100%;border-radius:99px;transition:.4s;"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-mis">
            ...
            <div class="card-mis-body">
                ...
                <form method="POST" style="margin-bottom:.6rem;">
                    ...
                    <button type="submit" onclick="return confirm('Approve this plan?')"
                ...
                <div id="rejectBox" style="display:none;margin-top:.75rem;">
                    <form method="POST">
                        ...
                        <button type="submit" onclick="return confirm('Reject this plan?')"
...
<div class="card-mis mb-4" style="border-top:3px solid #c9a84c;">
    <div class="card-mis-header">
        ...
        <div style="display:flex;gap:10px;font-size:.75rem;">
            <span style="color:#10b981;">● Logged <?= $matchedCount ?></span>
            <span style="color:#ef4444;">● Not Logged <?= $totalEntries - $matchedCount ?></span>
        </div>
    </div>
    <div class="table-responsive">
        <table class="table-mis w-100">
            ...
            <?php if (empty($entries)): ?>
                <tr>
                    <td colspan="14" style="text-align:center;padding:24px;color:#9ca3af;font-size:.83rem;">
            ...
            <?php foreach ($entries as $i => $e):
                $isMatched = (int)$e['logged_count'] > 0;
                ...
                $rowStyle = !$isMatched ? 'style="background:#fff9f9;"' : '';

                // Hour deviation indicator
                $plannedH = (float)$e['planned_hours'];
                $loggedH  = (float)$e['logged_hours'];
                $hDiff    = $plannedH > 0 ? $loggedH - $plannedH : 0;
                $hDiffCol = $hDiff >= 0 ? '#10b981' : '#ef4444';
                $hDiffTxt = $hDiff >= 0 ? '+'.number_format($hDiff,1).'h' : number_format($hDiff,1).'h';
            ?>
            <tr <?= $rowStyle ?>>
                ...
                <td class="text-center">
                    <span style="font-weight:600;color:#3b82f6;"><?= number_format($plannedH, 1) ?>h</span>
                </td>
                <!-- Actual times -->
                <td class="text-center" style="font-size:.78rem;">
                    <?php if ($isMatched && $e['actual_time_in']): ?>
                    <span style="color:#10b981;font-weight:500;"><?= date('g:i A', strtotime($e['actual_time_in'])) ?></span>
                    <?php else: ?>
                    <span style="color:#d1d5db;">—</span>
                    <?php endif; ?>
                </td>
                <td class="text-center" style="font-size:.78rem;">
                    <?php if ($isMatched && $e['actual_time_out']): ?>
                    <span style="color:#10b981;font-weight:500;"><?= date('g:i A', strtotime($e['actual_time_out'])) ?></span>
                    <?php else: ?>
                    <span style="color:#d1d5db;">—</span>
                    <?php endif; ?>
                </td>
                <td class="text-center">
                    <?php if ($isMatched): ?>
                    <div>
                        <span style="font-weight:700;color:<?= $loggedH >= 4 ? '#10b981' : ($loggedH >= 2 ? '#f59e0b' : '#ef4444') ?>;">
                            <?= number_format($loggedH, 1) ?>h
                        </span>
                    </div>
                    <?php if ($plannedH > 0): ?>
                    <div style="font-size:.63rem;color:<?= $hDiffCol ?>;font-weight:600;"><?= $hDiffTxt ?></div>
                    <?php endif; ?>
                    <?php else: ?>
                ...
                <!-- Notes -->
                <td style="font-size:.73rem;color:#6b7280;max-width:180px;word-wrap:break-word;white-space:normal;">
