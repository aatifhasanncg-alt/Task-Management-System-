...
if (!isCoreAdmin()) {
    setFlash('error', 'Access denied. Only Administrator or Branch Manager can manage grades.');
    header('Location: ' . APP_URL . '/manager/dashboard/index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    // NEW: validate numeric range before it ever reaches the DB
    $maxAllowed = 99999999.99; // match your column's actual precision, see note below
    if ($min !== '' && (!is_numeric($min) || abs((float)$min) > $maxAllowed)) {
        $errors[] = 'Min Revenue must be a valid number within an allowed range.';
    }
    if ($max !== '' && (!is_numeric($max) || abs((float)$max) > $maxAllowed)) {
        $errors[] = 'Max Revenue must be a valid number within an allowed range.';
    }
    if ($min !== '' && $max !== '' && is_numeric($min) && is_numeric($max) && (float)$min > (float)$max) {
        $errors[] = 'Min Revenue cannot be greater than Max Revenue.';
    }
    ...
    <div style="background:#fff;border-radius:16px;padding:1.75rem;
        ...
        <form method="POST" id="grade-form">
            ...
            <div class="row g-2 mb-3">
                <div class="col-6">
                    <label class="form-label-mis">Min Revenue</label>
                    <input type="number" step="0.01" name="min_profit" id="form-min"
                           class="form-control" placeholder="0.00">
                </div>
                <div class="col-6">
                    <label class="form-label-mis">Max Revenue</label>
