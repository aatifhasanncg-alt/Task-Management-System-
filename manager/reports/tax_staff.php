...
// ── Resolve Tax dept + Hetauda branch IDs ─────────────────────────────────────
$taxDeptRow = $db->query("
...
// ── Build dynamic status columns (mirrors report/index.php) ───────────────────
$statusCols = '';
foreach ($allStatuses as $st) {
...
// ── Staff query — Tax dept, Hetauda branch only (same logic as report/index.php) ──
$staffParams = array_merge(
    [$htdBranchId, $taxDeptId, $dateFrom, $dateTo],
    [$htdBranchId, $taxDeptId, $dateFrom, $dateTo],
    [$htdBranchId, $taxDeptId, $taxDeptId],   // +1 extra $taxDeptId
    $nameParams
);
...
$statusReport   = array_column($summaryStmt->fetchAll(), 'cnt', 'status');
$totalDeptTasks = array_sum($statusReport);
$doneCount      = 0;
foreach ($statusReport as $sn => $cnt) {
...
try {
    ...
    $tIn->execute([$htdBranchId, $taxDeptId, $dateFrom, $dateTo]);
    ...
    $tOut->execute([$htdBranchId, $taxDeptId, $dateFrom, $dateTo]);
    $transferOut = (int)$tOut->fetchColumn();
} catch (Exception $e) {}

// ── Donut helper (identical to report/index.php) ──────────────────────────────
...
.view-only-badge{
    ...
    border-radius:99px;font-size:.72rem;font-weight:700;padding:.3rem .8rem;
}
</style>
...
<div class="app-wrapper">
    ...
    <div class="main-content">
        ...
        <div style="padding:1.5rem 0;">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3"
                    ...
                    <div>
                        <div style="display:inline-flex;align-items:center;gap:.5rem;
                            ...
                            <i class="fas fa-users" style="font-size:.65rem;"></i> Staff Performance
                        </div>
            ...
            <div class="rpt-filter">
                <form method="GET" class="row g-2 align-items-end w-100">
                    ...
                    <div class="col-auto d-flex gap-1">
                        ...
                        <a href="tax_staff_htd.php" class="btn btn-outline-secondary btn-sm" title="Reset">
                ...
                <div class="col-12 col-md-6 col-xl-3">
                    <div class="completion-ring-card">
                        ...
                        <div>
                            ...
                            <div style="font-size:.7rem;color:<?= $deptColor ?>;margin-top:.35rem;font-weight:600;">
                                Tax Dept — Hetauda
                            </div>
                        </div>
                    </div>
            ...
            <!-- ── STAFF PERFORMANCE TABLE (mirrors report/index.php staffTable) ── -->
            <div class="chart-card mb-4">
                <div class="chart-card-header">
                    <div>
                        ...
                        <small style="color:#9ca3af;">
                            Tax Department · Hetauda Branch · <?= count($staffReport) ?> staff
                        </small>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <span class="view-only-badge">
                            <i class="fas fa-eye" style="font-size:.6rem;"></i> View Only
                        </span>
                    </div>
                </div>
                ...
                <div style="padding:3rem;text-align:center;color:#9ca3af;">
                    <i class="fas fa-users fa-2x mb-2 d-block"></i>No staff found for Tax dept · Hetauda.
                </div>
                <?php else: ?>
                ...
                <!-- ── Mobile card list ── -->
                <div class="d-md-none" style="padding:.75rem;">
                    ...
                    <div style="background:#fff;border:1px solid #f3f4f6;border-radius:12px;
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    <div style="font-weight:600;color:#1f2937;"><?= htmlspecialchars($s['full_name'] ?? '—') ?></div>
                                    <div style="font-size:.68rem;color:#9ca3af;"><?= htmlspecialchars($s['employee_id'] ?? '') ?></div>
                                </div>
                            </div>
                            <?= donut($donePct, $deptColor, 48) ?>
                        </div>
                        <div class="d-flex gap-2 flex-wrap">
                            <span style="background:#f3f4f6;color:#374151;font-size:.72rem;
                            ...
                            <?php foreach ($allStatuses as $st):
                                ...
                                $sbg  = $st['bg_color'] ?: '#f3f4f6';
                            ?>
                            <span style="background:<?= $sbg ?>;color:<?= $sc ?>;font-size:.72rem;
                                         padding:.2rem .55rem;border-radius:99px;font-weight:600;">
                ...
                <?php endif; // end if staffReport not empty ?>
            </div>
            ...
            <div class="chart-card mb-4">
                <div class="chart-card-header">
                    ...
                    <span style="font-size:.7rem;color:#9ca3af;">Stacked by status · Tax dept · Hetauda</span>
                </div>
                <div style="padding:1.25rem;">
                    <div style="height:300px;position:relative;">
                        <canvas id="staffChartTax"></canvas>
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <?php if (!empty($staffReport)): ?>
    (function () {
        const ctx = document.getElementById('staffChartTax');
        if (!ctx) return;

        const staff = <?= json_encode(array_map(function($s) use ($allStatuses) {
            $row = [
                'name'  => explode(' ', $s['full_name'])[0] ?? 'N/A',
                'total' => (int)$s['total'],
            ];
            foreach ($allStatuses as $st) {
                $safe = preg_replace('/[^a-zA-Z0-9_]/', '_', strtolower($st['status_name']));
