...
$isBranchManager = (($roleRow['dept_code'] ?? '') === 'CORE'); // admin + CORE dept = Branch Manager
$userBranchId = (int) ($roleRow['branch_id'] ?? 0);

$where = ['c.is_active = 1'];
$params = [];

if ($isBranchManager) {
    // Branch managers can optionally filter by branch, otherwise see all
    if ($filterB) {
        $where[] = 'c.branch_id = ?';
        $params[] = $filterB;
    }
} else {
    // Regular admins are hard-locked to their own branch
    if ($userBranchId) {
        $where[] = 'c.branch_id = ?';
        $params[] = $userBranchId;
    }
...
if ($isBranchManager) {
    ...
    $allBranches = $db->query("SELECT id, branch_name FROM branches WHERE is_active=1 AND id=" . $userBranchId)->fetchAll();
}
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="filter-bar mb-4 w-100">
                <form method="GET" class="row g-2 align-items-end w-100">
                    ...
                    <!--  Branch dropdown in the filter form -->
                    <?php if ($isBranchManager): ?>
                        <div class="col-md-2">
                    ...
                    <div class="col-md-3 d-flex align-items-end gap-1">
                        ...
                        <a href="company_wise.php" class="btn btn-outline-secondary btn-sm"><i
            ...
            <?php if (empty($companies)): ?>
                <div class="card-mis">
                    <div class="card-mis-body text-center py-5" style="color:#9ca3af;"><i
                            class="fas fa-building fa-2x mb-2 d-block"></i>
            ...
            <?php else: ?>
                ...
                <div class="card-mis mb-3 p-3">
                    <div class="d-flex gap-4 flex-wrap" style="font-size:.82rem;">
                        <?php $totalTasks = array_sum(array_column($companies, 'task_count'));
                        ...
                        <div class="company-card card-mis mb-2" id="co-<?= $co['id'] ?>">
                            <div class="company-row" onclick="toggleCompany(<?= $co['id'] ?>)"
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    <div
                                        style="width:40px;height:40px;border-radius:10px;background:#0a0f1e;color:#c9a84c;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.8rem;flex-shrink:0;">
                                    ...
                                    <div>
                                        ...
                                        <div class="d-flex gap-3 flex-wrap"
                                            ...
                                            <?php if ($co['company_code']): ?><span><i
                                                        class="fas fa-hashtag me-1"></i><?= htmlspecialchars($co['company_code']) ?></span><?php endif; ?>
                                            <?php if ($co['pan_number']): ?><span><i
                                                        class="fas fa-id-card me-1"></i><?= htmlspecialchars($co['pan_number']) ?></span><?php endif; ?>
                                            <?php if ($co['branch_name']): ?><span><i
                                                        class="fas fa-map-marker-alt me-1"></i><?= htmlspecialchars($co['branch_name']) ?></span><?php endif; ?>
                                            <?php if ($co['company_type_name']): ?><span><i
                                                        class="fas fa-tag me-1"></i><?= htmlspecialchars($co['company_type_name']) ?></span><?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="d-flex align-items-center gap-3">
                                    <?php if ($co['task_count'] > 0): ?>
                                        <div style="min-width:80px;">
                                            <div class="d-flex justify-content-between mb-1"
                                                ...
                                                <span><?= $co['done_count'] ?>/<?= $co['task_count'] ?></span><span><?= $completionPct ?>%</span>
                                            </div>
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <?php if ($co['task_count'] > 0): ?><span
                                                class="badge bg-primary-soft text-primary fw-semibold"><?= $co['task_count'] ?>
                                                tasks</span><?php else: ?><span class="badge bg-light text-muted">No
                                                tasks</span><?php endif; ?>
                                        <?php if ($hasOverdue): ?><span class="badge bg-danger-soft text-danger fw-semibold">⚠
                                                <?= $co['overdue_count'] ?> overdue</span><?php endif; ?>
                                    ...
                                    <i class="fas fa-chevron-down chevron-icon" id="chev-<?= $co['id'] ?>"
                                        style="color:#9ca3af;font-size:.75rem;transition:transform .25s;<?= $isExpanded ? 'transform:rotate(180deg);' : '' ?>"></i>
                                </div>
                            </div>
                            <div class="workflow-panel" id="panel-<?= $co['id'] ?>"
                                style="display:<?= $isExpanded ? 'block' : 'none' ?>;border-top:1px solid #f3f4f6;">
                                <div class="workflow-inner p-3" id="inner-<?= $co['id'] ?>">
                                    <div class="text-center py-4" style="color:#9ca3af;font-size:.83rem;"><i
                                            class="fas fa-spinner fa-spin me-1"></i> Loading workflow…</div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
