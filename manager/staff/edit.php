...
if (!isCoreAdmin()) {
    setFlash('error', 'Access denied. Only Administrator or Branch Manager can edit staff.');
...
// Fetch existing UDA departments with managed_by
$udaFullStmt = $db->prepare("
    SELECT uda.department_id, uda.managed_by, d.dept_name
    FROM user_department_assignments uda
    JOIN departments d ON d.id = uda.department_id
    WHERE uda.user_id = ?
      AND uda.department_id != ?
");
$udaFullStmt->execute([$staffId, $staff['department_id']]);
$udaFull = $udaFullStmt->fetchAll(PDO::FETCH_ASSOC);
$allAdmins = $db->query("
        ...
        foreach ($extraDepts as $extraDeptId) {
            ...
            if ($extraDeptId && $extraDeptId !== $deptId) {
                ...
                $db->prepare("
                    ...
                    VALUES (?, ?, ?)
                ")->execute([$staffId, $extraDeptId, $deptManagedBy]);
            }
        }
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <div style="font-size:.78rem;color:#9ca3af;margin-top:.15rem;">
                                ...
                                <span
                                    style="text-transform:capitalize;"><?= htmlspecialchars($currentRoleName) ?></span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-mis-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <small style="font-size:.65rem;color:#9ca3af;">Auto-assigned, cannot be
                                            changed</small>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div style="display:flex;gap:1.5rem;flex-wrap:wrap;margin-top:.25rem;">
                                            ...
                                            <label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;
                                              ...
                                              padding:.55rem .9rem;min-width:160px;transition:.15s;" id="activeToggle">
                                                <div style="position:relative;width:36px;height:20px;flex-shrink:0;">
                                                    ...
                                                    <div class="toggle-track" id="activeTrack" style="width:36px;height:20px;border-radius:99px;
                                                    background:<?= $staff['is_active'] ? '#10b981' : '#d1d5db' ?>;
                                                    transition:.2s;position:relative;">
                                                        <div style="position:absolute;top:2px;
                                                        left:<?= $staff['is_active'] ? '18px' : '2px' ?>;
                                                        width:16px;height:16px;border-radius:50%;
                                                        background:#fff;transition:.2s;" id="activeThumb"></div>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div style="font-size:.82rem;font-weight:600;color:#374151;">Active
                                                        User</div>
                                                    <div style="font-size:.68rem;color:#9ca3af;">Can log in to the
                                                        system</div>
                                                </div>
                                            </label>
                                            ...
                                            <label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;
                                              background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;
                                              padding:.55rem .9rem;min-width:160px;transition:.15s;" id="gaToggle">
                                                <div style="position:relative;width:36px;height:20px;flex-shrink:0;">
                                                    <input type="checkbox" name="ga_enabled" id="gaCheck"
                                                        style="opacity:0;position:absolute;width:0;height:0;"
                                                        <?= $staff['ga_enabled'] ? 'checked' : '' ?>>
                                                    <div class="toggle-track" id="gaTrack" style="width:36px;height:20px;border-radius:99px;
                                                    background:<?= $staff['ga_enabled'] ? '#3b82f6' : '#d1d5db' ?>;
                                                    transition:.2s;position:relative;">
                                                        <div style="position:absolute;top:2px;
                                                        left:<?= $staff['ga_enabled'] ? '18px' : '2px' ?>;
                                                        width:16px;height:16px;border-radius:50%;
                                                        background:#fff;transition:.2s;" id="gaThumb"></div>
                                                    </div>
                                                </div>
                                                <div>
                                                    ...
                                                    <div style="font-size:.68rem;color:#9ca3af;">Google Authenticator
                                                    </div>
                                                </div>
                                            </label>
                            ...
                            <div class="card-mis-body" style="overflow:visible;">
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select name="managed_by" class="form-select tomselect-input"
                                            id="managedBySelect">
                                            ...
                                            <?php foreach ($allAdmins as $a): ?>
                                                ...
                                                if (!empty($a['employee_id']))
                                                    $adminMeta[] = $a['employee_id'];
                                                if (!empty($a['branch_name']))
                                                    $adminMeta[] = $a['branch_name'];
                            ...
                            <div class="card-mis-body" style="font-size:.8rem;color:#6b7280;">
                                ...
                                padding:.35rem 0;border-bottom:1px solid #f3f4f6;">
                                        <div style="display:flex;align-items:center;gap:.5rem;color:#9ca3af;">
                                            <i class="fas <?= $icon ?>"
                                                style="font-size:.7rem;width:12px;text-align:center;"></i>
                                            <span><?= $label ?></span>
                                        </div>
                                ...
                                <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.75rem;">
                                    <span style="background:<?= $staff['is_active'] ? '#ecfdf5' : '#f9fafb' ?>;
                                     ...
                                     padding:.2rem .6rem;border-radius:99px;font-size:.7rem;font-weight:600;">
                                        <i
                                            class="fas <?= $staff['is_active'] ? 'fa-circle-check' : 'fa-circle-xmark' ?> me-1"></i>
                        ...
                        <div class="card-mis p-3" style="border-left:3px solid #3b82f6;">
                            <div style="font-size:.78rem;color:#374151;">
                                ...
                                <div style="color:#6b7280;line-height:1.7;">
                                    All fields are editable except <strong>Employee ID</strong> which is
                                    system-assigned.
                                    Changing the <strong>Role</strong> will trigger a permission reset via the role
                                    manager.
