...
// Always compute full accessible dept list for non-core-admin (used by dropdown scoping)
$allAccessDeptIds = [];
if (!$isCoreAdminDept) {
    $udaAllStmt2 = $db->prepare("SELECT department_id FROM user_department_assignments WHERE user_id = ?");
    $udaAllStmt2->execute([$userId]);
    $allAccessDeptIds = array_unique(array_filter(
        array_merge([$myDeptId], array_column($udaAllStmt2->fetchAll(), 'department_id'))
    ));
}
...
if ($isCoreAdminDept) {
    ...
    // Non-core-admin: scope by active UDA tab or all accessible depts — any role
    if (count($staffDeptTabs) > 1 && $activeStaffDeptTab) {
        ...
        $params = array_merge($params, $allAccessDeptIds, $allAccessDeptIds);
    }
}
...
$allBranches = $isCoreAdminDept
    ...
    : [];
// ── Employee dropdown (TomSelect) — scoped same as staff list ────────────────
// ── Employee dropdown (TomSelect) ─────────────────────────────────────────────
// Core-admin (ADM/CORE dept): all staff in own branch, any role.
// Regular dept user: all staff matching own department or UDA access, any role.
$empWhere = ['u.is_active = 1'];
$empParams = [];

if ($isCoreAdminDept) {
    $empWhere[] = 'u.branch_id = ?';
    $empParams[] = $myBranchId;
} else {
    if (!empty($allAccessDeptIds)) {
        $deptPh = implode(',', array_fill(0, count($allAccessDeptIds), '?'));
        $empWhere[] = '(
        ...
        $empParams = array_merge($empParams, $allAccessDeptIds, $allAccessDeptIds);
    } else {
        // No dept/UDA access at all — fall back to self only
        $empWhere[] = 'u.id = ?';
        $empParams[] = $userId;
    }
}
...
$empDropdownStmt = $db->prepare("
    ...
    FROM users u
    LEFT JOIN roles r ON r.id = u.role_id
    WHERE {$empWs} AND (r.role_name != 'executive' OR u.id = ?)
    ORDER BY u.full_name
");
$empDropdownStmt->execute(array_merge($empParams, [$userId]));
                        ...
                        <select name="emp_filter" id="empFilterSelect" class="form-select form-select-sm">
                            ...
                            <?php foreach ($empDropdownList as $emp): ?>
                                <option value="<?= $emp['id'] ?>" <?= $filterEmp == $emp['id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($emp['full_name']) ?>    <?= $emp['employee_id'] ? ' (' . htmlspecialchars($emp['employee_id']) . ')' : '' ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($staffList as $u):
                                $pct = $u['task_total']
                                    ...
                                    : 0;
                                $roleColors = ['admin' => '#3b82f6', 'staff' => '#10b981', 'manager' => '#8b5cf6', 'executive' => '#8b5cf6'];
                                ...
                                <tr <?= $isSelf ? 'style="background:#fffbeb;"' : '' ?>>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="<?= APP_URL ?>/manager/staff/view.php?id=<?= $u['id'] ?>"
                                                class="btn btn-sm btn-outline-secondary" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <?php if ($isCoreAdminDept): ?>
                                                <a href="<?= APP_URL ?>/manager/staff/edit.php?id=<?= $u['id'] ?>"
