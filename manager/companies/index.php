...
// ── Viewer dept / CORE detection (for task count scoping) ──────────────────
$user = currentUser();
$viewerDeptId = (int) ($user['department_id'] ?? 0);
$isCore = isCoreAdmin(); // CORE/branch-manager sees full counts across all depts
...
$cos = $db->prepare("
    SELECT c.*,
           ...
           (SELECT COUNT(*)
            ...
            " . (!$isCore ? "AND t.department_id = {$viewerDeptId}" : "") . "
            ) AS task_count,
            (SELECT COUNT(*)
            FROM tasks t
