...
if (in_array($currentRole, ['staff', 'admin', 'executive', 'manager'])) {
    $isAdmin = true;
} else {
    $isStaff = true;
}
...
/* POST: save_it — manager can create OR update task_it for this task */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_it']) && $canEditDept) {
    ...
    $reportedDate = trim($it['reported_date'] ?? '') ?: null;
    ...
    if ($itExisting) {
        if ($tokenRaiserId === null) {
            $tokenRaiserId = ($itExisting['token_raiser_id'] ?? null) !== null
                ? (int) $itExisting['token_raiser_id']
                : null;
        }

        if ($tokenRaiserId === null) {
            $tokenRaiserId = (int) ($task['created_by'] ?? 0) ?: (int) ($user['id'] ?? 0);
        }
        ...
        try {
            ...
            $raiserId = $tokenRaiserId ?: (int) ($task['created_by'] ?? 0) ?: (int) ($user['id'] ?? 0);
            if (!$raiserId) {
                setFlash('error', 'Could not determine ticket raiser. Please contact an administrator.');
                header("Location: view.php?id={$id}");
                exit;
            }
...
$sidebarFile = '../../includes/sidebar_manager.php';
        ...
        <div style="padding:1.5rem 0;">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    ...
                    <?php elseif ($task['dept_code'] === 'IT' && ($isCoreAdminDept || $canViewDept)): ?>
                        <div class="vw-section">
                            <div class="vw-section-header">
                                <h5><i class="fas fa-laptop-code text-warning me-2"></i>IT Support Details</h5>
                                <?php if (!$canEditDept): ?>
                                    <span
                            ...
                            <div class="vw-section-body">
                                ...
                                <?php if ($detail): ?>
                                    ...
                                    <div class="row g-3 mb-4">
                                        ...
                                        $itViewRows = [
                                            'Token Number' => '<code style="background:#f3f4f6;padding:.2rem .5rem;border-radius:5px;font-size:.88rem;">' . htmlspecialchars($detail['token_number'] ?? '—') . '</code>',
                                            ...
                                            'Reported Date' => ($detail['reported_date'] ?? null) ? date('d M Y, H:i', strtotime($detail['reported_date'])) : '—',
                                            'Resolution Date' => ($detail['resolution_date'] ?? '') ? date('d M Y, H:i', strtotime($detail['resolution_date'])) : '—',
                                        ];
                                        foreach ($itViewRows as $lbl => $val): ?>
                                            <div class="col-md-4">
                                                <div class="vw-label"><?= $lbl ?></div>
                                                <div class="vw-value"><?= $val ?></div>
                                            </div>
                                        ...
                                        <?php if ($detail['detailed_description'] ?? ''): ?>
                                            <div class="col-12">
                                                <div class="vw-label">Detailed Description</div>
                                                <div class="vw-value"
                                ...
                                <?php elseif (!$canEditDept): ?>
                                    <div class="text-center py-4 text-muted">
                                        <i class="fas fa-file-circle-question fa-2x mb-2 d-block opacity-50"></i>
                                        No IT details yet.
                                    </div>
                                <?php endif; ?>

                                <!-- ── EDIT ── -->
                                <?php if ($canEditDept && $detail): ?>
                                    <div
                                        style="font-size:.8rem;font-weight:700;color:#6b7280;text-transform:uppercase;margin-bottom:.75rem;">
                                        <i class="fas fa-pen me-1"></i>Update IT Details
                                    </div>
                                    <form method="POST">
                                        <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <!-- issue_category: ENUM NOT NULL -->
                                            <div class="col-md-6">
                                                <label class="form-label-mis">Issue Category <span
                                                        class="req-star">*</span></label>
                                                <select name="it[issue_category]" class="form-select form-select-sm" required>
                                                    <?php foreach ([
                                                        'Task System',
                                            ...
                                            <!-- assigned_it_staff: nullable FK → users -->
                                            <div class="col-md-3">
                                                <label class="form-label-mis">Assign IT Staff</label>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <textarea name="it[detailed_description]" class="form-control form-control-sm"
                                                    rows="3"
                                                    placeholder="Describe the issue in detail…"><?= htmlspecialchars($detail['detailed_description'] ?? '') ?></textarea>
                                            </div>

                                            <!-- resolution: nullable TEXT -->
                                            <div class="col-12">
                                                <label class="form-label-mis">Resolution</label>
                                            ...
                                            <div class="col-md-4">
                                                <label class="form-label-mis">Resolution Date & Time</label>
                                                <input type="datetime-local" name="it[resolution_date]"
                                                    class="form-control form-control-sm" value="<?= htmlspecialchars(
                                                        $detail['resolution_date']
                                                        ? date('Y-m-d\TH:i', strtotime($detail['resolution_date']))
                                                        : ''
                                                    ) ?>">
                                            </div>
                                            ...
                                            <!-- Read-only info -->
                                            <div class="col-md-4">
                                                <label class="form-label-mis">Token Number</label>
                                                <div class="form-control form-control-sm"
                                ...
                                <?php elseif ($canEditDept && !$detail): ?>
                                    <div
                                        style="font-size:.8rem;font-weight:700;color:#6b7280;text-transform:uppercase;margin-bottom:.75rem;">
                                        <i class="fas fa-pen me-1"></i>Add IT Issue Details
                                    </div>
                                    <form method="POST">
                                        <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                                        <input type="hidden" name="save_it" value="1">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <select name="it[issue_category]" class="form-select form-select-sm" required>
                                                    <?php foreach ([
                                                        ...
                                                        <option value="<?= htmlspecialchars($cat) ?>"><?= htmlspecialchars($cat) ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <select name="it[severity]" class="form-select form-select-sm">
                                                    <?php foreach (['Low', 'Medium', 'High', 'Critical'] as $s): ?>
                                                        <option value="<?= $s ?>" <?= $s === 'Medium' ? 'selected' : '' ?>><?= $s ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            ...
                                            <div
                                                ...
                                                <div style="display:flex;flex-wrap:wrap;gap:.5rem;">
                                                    <?php foreach ($taskStatuses as $ts):
                                                        ...
                                                        <label style="cursor:pointer;">
                                                            <input type="radio" name="main_status"
                                                                value="<?= htmlspecialchars($ts['status_name']) ?>" <?= $isChecked ? 'checked' : '' ?> style="display:none;" class="status-radio-input">
                                                            <span class="status-pill-option" data-bg="<?= htmlspecialchars($sBg) ?>"
                                                                style="background:<?= $isChecked ? $sBg : '#fff' ?>;color:<?= $sCol ?>;border:1.5px solid <?= $sCol ?>;padding:.3rem .8rem;border-radius:99px;font-size:.78rem;font-weight:600;display:inline-block;">
                                                                <?= htmlspecialchars($ts['status_name']) ?>
                                                            </span>
                                                        </label>
    ...
    document.querySelectorAll('.status-radio-input').forEach(radio => {
        radio.addEventListener('change', function () {
            ...
            group.querySelectorAll('.status-radio-input').forEach(r => {
                ...
                pill.style.background = r.checked ? pill.dataset.bg : '#fff';
            });
        });
    });
