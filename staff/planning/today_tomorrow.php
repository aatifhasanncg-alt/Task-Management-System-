...
{
    $weekStart = date('Y-m-d', strtotime('monday this week', strtotime($planDate)));
    $weekEnd = date('Y-m-d', strtotime('sunday this week', strtotime($planDate)));
    $planMonth = date('Y-m-01', strtotime($planDate));
    $weekNumber = (int) ceil(date('j', strtotime($planDate)) / 7);

    $stmt = $db->prepare("SELECT id FROM work_plans WHERE user_id=? AND week_start_date=? LIMIT 1");
    $stmt->execute([$uid, $weekStart]);
    $planId = $stmt->fetchColumn();
    if ($planId)
        return (int) $planId;

    // Fetch user's required fields
            ...
            <div class="page-hero mb-4">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-center">
                        ...
                        <button type="button" class="btn btn-gold btn-sm" onclick="openPlanModal()">
