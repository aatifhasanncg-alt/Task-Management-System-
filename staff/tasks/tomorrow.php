...
$tasks = $db->prepare("
    SELECT t.*, d.dept_name, d.color as dept_color, d.icon as dept_icon,
           ...
           ts.bg_color AS status_bg, ts.icon AS status_icon
    FROM tasks t
                            ...
                            <div class="d-flex align-items-start justify-content-between gap-3">
                                <div>
                                    <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                                        ...
                                        <span style="background:<?= htmlspecialchars($t['status_bg'] ?? '#f3f4f6') ?>;
                                            color:<?= htmlspecialchars($t['status_color'] ?? '#9ca3af') ?>;
                                    ...
                                    <h5 style="font-size:.95rem;font-weight:600;margin:.25rem 0;">
                                        <?= htmlspecialchars($t['title']) ?></h5>
                                    <div style="font-size:.78rem;color:#9ca3af;">
                                        <?php if ($t['company_name']): ?><span class="me-3"><i
                                                    class="fas fa-building me-1"></i><?= htmlspecialchars($t['company_name']) ?></span><?php endif; ?>
                                        <span><i class="fas fa-flag me-1"
                                                style="color:<?= ['urgent' => '#ef4444', 'high' => '#f59e0b', 'medium' => '#3b82f6', 'low' => '#9ca3af'][$t['priority']] ?? '#9ca3af' ?>;"></i><?= ucfirst($t['priority']) ?></span>
                                    </div>
                                </div>
