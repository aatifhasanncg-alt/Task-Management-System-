...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_tax']) && $task['dept_code'] === 'TAX') {
    ...
    try {
        if ($exists) {
            // 20 SET columns + 1 WHERE = 21 ? placeholders
            $db->prepare("
                UPDATE task_tax SET
                    ...
                    tax_clearance_status_id=?, total_amount=?, completed_date=?,
                    remarks=?, notes=?
                WHERE task_id=?
            ")->execute(array_merge($p, [$id]));
        } else {
            // task_id + 20 columns = 21 ? placeholders
            $db->prepare("
...
if (
    $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_banking'])
    ...
    foreach (['fiscal_year VARCHAR(10) NULL AFTER completion_date', 'fiscal_year_id INT NULL AFTER fiscal_year'] as $colDef) {
        ...
        try {
            $db->query("SELECT $colName FROM task_banking LIMIT 1");
        } catch (Exception $e) {
            try {
                $db->exec("ALTER TABLE task_banking ADD COLUMN $colDef");
            } catch (Exception $e2) {
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['transfer_staff'])) {
    ...
    notify(
        $newStaffId,
        "Task Assigned: {$task['task_number']}",
        $staffMsg,
        'transfer',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['transfer_department'])) {
    ...
    if (!$newDept) {
        setFlash('error', 'Invalid department selection.');
        header("Location: view.php?id={$id}");
        exit;
    }
    ...
    if ($newAssignTo && $newAssignUser) {
        ...
        notify(
            $newAssignTo,
            "Task Transferred to You: {$task['task_number']}",
            $deptMsg,
            'transfer',
            APP_URL . '/staff/tasks/view.php?id=' . $id,
            true,
    ...
    if (!empty($task['created_by']) && $task['created_by'] != $user['id']) {
        ...
        notify(
            (int) $task['created_by'],
            "Task Dept Transferred: {$task['task_number']}",
            $adminMsg,
            'transfer',
            APP_URL . '/admin/tasks/view.php?id=' . $id,
            false
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_comment'])) {
    ...
    if ($comment) {
        try {
            $db->prepare("INSERT INTO task_comments(task_id,user_id,comment)VALUES(?,?,?)")->execute([$id, $user['id'], $comment]);
        } catch (Exception $e) {
        }
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                                ...
                                <?php if ($detail): ?>
                                    <div class="row g-3 mb-4">
                                        ...
                                        foreach ([
                                            ...
                                            'Follow-up Date' => !empty($followupHistory) ? date('d M Y', strtotime(end($followupHistory)['followup_date'])) : '—',
                                        ] as $lbl => $val): ?>
                                            <div class="col-md-4">
                                                <div
                                                    style="font-size:.72rem;font-weight:700;color:#9ca3af;text-transform:uppercase;">
                                                ...
                                                <div class="d-flex align-items-center gap-2 mt-1">
                                                    ...
                                                    <?php if ($val): ?>
                                                        <a href="<?= $url ?>" target="_blank"
                                                            style="background:<?= $col ?>;color:white;padding:.2rem .6rem;border-radius:6px;font-size:.72rem;text-decoration:none;">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="number" name="tax[total_amount]"
                                                ...
                                                value="<?= htmlspecialchars($detail['total_amount'] ?? '0') ?>">
                                        </div>
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            <label class="form-label-mis">Updated By <span
                                                    style="font-size:.65rem;color:#8b5cf6;margin-left:.3rem;"><i
                                                        class="fas fa-filter me-1"></i>Tax dept only</span></label>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" name="tax[follow_up_note]"
                                                class="form-control form-control-sm"
                                                placeholder="e.g. Called client, waiting for docs...">
                                        </div>
                                        <div class="col-12">
                    ...
                    <?php elseif ($task['dept_code'] === 'RETAIL' && $detail): ?>
                        ...
                        <!-- ════ BANK DETAILS — read-only for all non-Nikita banking staff ════ -->
                    <?php elseif ($task['dept_code'] === 'BANK' && !$isNikitaBank && $detail): ?>
                        <div class="card-mis mb-4">
                            <div class="card-mis-header">
                            ...
                            <div class="card-mis-body">
                                <div class="row g-3">
                                    <?php foreach ([
                                        ...
                                        'Term Loan' => $detail['term'] !== null ? 'Rs. ' . number_format($detail['term'], 2) . 'L' : '—',
                                        ...
                                        <?php if (!empty($detail['remarks'])): ?>
                                            <div class="mt-2">
                                                <div
                                                    style="font-size:.68rem;font-weight:700;color:#9ca3af;text-transform:uppercase;">
                                                    Remarks</div>
                                                <div style="font-size:.87rem;"><?= nl2br(htmlspecialchars($detail['remarks'])) ?>
                                                </div>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <form method="POST">
                                                ...
                                                <?php foreach ($allBanks as $bk): ?>
                                                    <option value="<?= $bk['id'] ?>" <?= ($detail['bank_reference_id'] ?? '') == $bk['id'] ? 'selected' : '' ?>>
                                                        ...
                                                        <?php if (!empty($bk['address'])): ?> -
                                                            <?= htmlspecialchars($bk['address']) ?>         <?php endif; ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    ...
                                    <div style="border-top:1px solid #f3f4f6;padding-top:1rem;margin-top:1rem;">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <?php endforeach; ?>
                                            <div class="col-md-3 col-6">
                                                <label class="form-label-mis">OD (Rs.) <small class="text-muted">in
                                                        lakh</small></label>
                                            ...
                                            <div class="col-md-3 col-6">
                                                <label class="form-label-mis">Term Loan (Rs.) <small class="text-muted">in
                                                        lakh</small></label>
                                        ...
                                        <div
                                            ...
                                            <div style="font-size:.75rem;color:#6b7280;margin-top:.1rem;">
                                                ...
                                                <?php if (!empty($w['from_dept']) && !empty($w['to_dept']) && $w['from_dept'] !== $w['to_dept']): ?>
                                                    · <?= htmlspecialchars($w['from_dept']) ?> <i class="fas fa-arrow-right mx-1"
                                                        style="font-size:.65rem;"></i> <?= htmlspecialchars($w['to_dept']) ?>
                                                <?php endif; ?>
                                            </div>
                ...
                <div class="col-lg-4">
                    ...
                    <?php if ($isMyTask): ?>
                                                ...
                                                <?php foreach ($taskStatuses as $ts):
                                                    ...
                                                    <label style="display:block;cursor:pointer;margin-bottom:.5rem;">
                                                        ...
                                                        <div class="status-radio-tile <?= $isChecked ? 'is-selected' : '' ?>"
                                                            ...
                                                            <span
                                                                style="font-size:.8rem;font-weight:600;color:<?= htmlspecialchars($sc) ?>;flex:1;"><?= htmlspecialchars($ts['status_name']) ?></span>
                                                            <?php if ($isChecked): ?><span
                                                                    style="font-size:.65rem;color:#9ca3af;font-weight:400;">current</span><?php endif; ?>
                                                        </div>
                                                    </label>
                                                <?php endforeach; ?>
                                            </div>
                        ...
                        <?php else: ?>
                            ...
                            <div class="card-mis mb-3" style="border-left:3px solid #10b981;">
                                <div class="card-mis-body text-center py-3">
                                    ...
                                    <div style="font-size:.78rem;color:#9ca3af;margin-top:.3rem;">You can still transfer this
                                        task below.</div>
                                </div>
                            </div>
                        <?php endif; ?>
                                ...
                                <div class="card-mis-body">
                                    ...
                                    <form method="POST" onsubmit="return confirm('Transfer this task to selected staff?');">
                                        ...
                                        <div class="mb-2">
                                            ...
                                            <select name="new_staff_id" id="staff_transfer_select"
                        ...
                        <?php if (!empty($branchDepts)): ?>
                            <div class="card-mis mb-3" style="border-left:3px solid #8b5cf6;">
                                ...
                                <div class="card-mis-body">
                                    <p style="font-size:.77rem;color:#9ca3af;margin-bottom:.75rem;">
                                        Transfer this task to the Finance department in
                                        <strong><?= htmlspecialchars($task['branch_name'] ?? 'your branch') ?></strong>.
                                    ...
                                    <form method="POST" onsubmit="return confirm('Transfer this task to Finance department?');">
                                        ...
                                        <input type="hidden" name="new_dept_id"
                                            value="<?= htmlspecialchars($branchDepts[0]['id']) ?>">
                                        <div
                                            style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:8px;padding:.5rem .75rem;margin-bottom:.75rem;font-size:.82rem;color:#6d28d9;font-weight:600;">
                                            <i
                                                class="fas fa-coins me-1"></i><?= htmlspecialchars($branchDepts[0]['dept_name']) ?>
                                        </div>
                                        <div class="mb-2">
                                            <label class="form-label-mis">Assign To (optional)</label>
                                            <select name="transfer_dept_staff" id="dept_staff_select"
                                                class="form-select form-select-sm">
                                                <option value="">-- Unassigned / dept admin decides --</option>
                                                <?php foreach ($branchAllStaff as $s):
                                                    if ((int) $s['department_id'] !== (int) $branchDepts[0]['id'])
                                                        continue; ?>
                                                    <option value="<?= $s['id'] ?>">
                                                        <?= htmlspecialchars($s['full_name']) ?>
                                                        <?= $s['employee_id'] ? ' (' . $s['employee_id'] . ')' : '' ?>
                                        ...
                                        <div class="mb-2">
                                            <label class="form-label-mis">Transfer Reason <span
                                            ...
                                            <div style="font-size:.68rem;color:#9ca3af;margin-top:.3rem;">
                                                <i class="fas fa-envelope me-1"></i>This reason will be sent to the assigned
                                                person via email and app notification.
                                            </div>
                                        </div>
                                        <button type="submit" class="btn w-100 btn-sm"
                    ...
                    <div class="card-mis p-3" style="font-size:.8rem;color:#6b7280;border-left:3px solid var(--gold);">
                        ...
                        <div class="mb-2"><strong>Department:</strong>
                            <?= htmlspecialchars($task['dept_name'] ?? '—') ?></div>
                    ...
                    <!-- Follow-up History (sidebar) -->
                    <?php if (!empty($followupHistory)): ?>
                        <div class="card-mis mt-3" style="border-left:3px solid #f59e0b;">
                            <div class="card-mis-header">
                                <h5><i class="fas fa-clock-rotate-left text-warning me-2"></i>Follow-up History</h5>
                                <span style="background:#f59e0b;color:white;padding:.15rem .5rem;
                         border-radius:99px;font-size:.68rem;font-weight:700;">
                                    <?= count($followupHistory) ?>
                                </span>
                            </div>
                            <div class="card-mis-body" style="max-height:300px;overflow-y:auto;">
                                <?php foreach ($followupHistory as $i => $fu): ?>
                                    <div class="d-flex gap-2 mb-3">
                                        <div style="width:22px;height:22px;border-radius:50%;background:#f59e0b;
                                color:white;display:flex;align-items:center;justify-content:center;
                                font-size:.65rem;font-weight:700;flex-shrink:0;">
                                            <?= $i + 1 ?>
                                        </div>
                                        <div style="background:#fffbeb;border:1px solid #fde68a;
                                border-radius:8px;padding:.45rem .7rem;flex:1;">
